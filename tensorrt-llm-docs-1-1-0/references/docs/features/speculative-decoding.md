Original URL: https://nvidia.github.io/TensorRT-LLM/1.1.0/features/speculative-decoding.md
...
### Draft/Target[#](#draft-target "Link to this heading")
...
### EAGLE 3[#](#eagle-3 "Link to this heading")
...
### NGram[#](#ngram "Link to this heading")
...
### User-provided drafting[#](#user-provided-drafting "Link to this heading")

A completely user-defined drafting method can be supplied with a `UserProvidedDecodingConfig` that includes

* `max_draft_len`: Maximum draft candidate length.
* `drafter`: An object of type `Drafter` that implements the `prepare_draft_tokens` method (see [Developer Guide](#developer-guide) 7.)
* `resource_manager`: An optional `ResourceManager` object (see [Developer Guide](#developer-guide) 4.)
...
## Usage with `trtllm-bench` and `trtllm-serve`[#](#usage-with-trtllm-bench-and-trtllm-serve "Link to this heading")
...
## Developer Guide[#](#developer-guide "Link to this heading")
...
## Two Model Speculative Decoding Architecture[#](#two-model-speculative-decoding-architecture "Link to this heading")
...
### `_prepare_draft_requests`[#](#prepare-draft-requests "Link to this heading")
...
### `_prepare_draft_tokens`[#](#prepare-draft-tokens "Link to this heading")
...
### Verification and Sampling[#](#verification-and-sampling "Link to this heading")
...
### KV Cache Rewind[#](#kv-cache-rewind "Link to this heading")
