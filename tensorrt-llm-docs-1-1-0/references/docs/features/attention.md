Original URL: https://nvidia.github.io/TensorRT-LLM/1.1.0/features/attention.md

* Multi-Head, Multi-Query, and Group-Query Attention

# Multi-Head, Multi-Query, and Group-Query Attention[#](#multi-head-multi-query-and-group-query-attention "Link to this heading")
...
## Implement a New Attention Backend[#](#implement-a-new-attention-backend "Link to this heading")
...
### Implement `AttentionMetadata`[#](#implement-attentionmetadata "Link to this heading")
...
| --- | --- | --- |
| max\_num\_requests | int | The max number of requests in a single batch. |
| num\_contexts | int | The number of context-phase sequences in the batch. |
| num\_generations | int | The number of generation-phase sequences in the batch. |
| max\_num\_tokens | int | The max number of tokens in all requests in a single batch. |
| num\_tokens | int | Number of tokens in the batch. |
| num\_ctx\_tokens | int | Number of tokens in sequences in the context phase. |
| kv\_cache\_manager | KVCacheManager | The KV cache manager. |
| is\_cuda\_graph | bool | Whether CUDA graph is enabled. |
| seq\_lens | Tensor | The length of each sequence in the batch. The shape is (batch\_size), and located on CPU memory. |
| seq\_lens\_cuda | Tensor | A copy of `seq_lens` store on the GPU. |
| context\_lens | Tensor | The length of each context-phase sequence in the batch. The shape is (`num_contexts`). |
| position\_ids | Optional[Tensor] | The position of each token in each sequence. May be None if positional embedding is applied outside of the backend. |
| request\_ids | List[int] | The request ID of each sequence in the batch. |
| prompt\_lens | List[int] | The prompt length of each sequence in the batch. |
| kv\_cache\_params | KVCacheParams | The parameters for the KV cache. |
...
### Implement `AttentionBackend`[#](#implement-attentionbackend "Link to this heading")
...
| Field | Type | Description |
| --- | --- | --- |
| layer\_idx | int | The index of the attention layer in the model. |
| num\_heads | int | The number of query heads. |
| head\_dim | int | The size of each attention head `(hidden_size // num_heads)`. |
| num\_kv\_heads | Optional[int] | The number of KV heads. Defaults to num\_heads if None. |
| quant\_config | QuantConfig | Optional quantization configuration. If None, no quantization is applied. |
| pos\_embd\_params | PositionalEmbeddingParams | Optional parameters defining how positional embedding should be applied. If None, positional embedding should be applied by the model before calling the backend. Otherwise, the backend is in-charge of applying positional embedding and may cache K without embedding it first. |
...
| attention\_mask | AttentionMask | Optional attention mask. If None, causal mask is applied. |
...
## The Features of the `TrtllmAttention` Backend[#](#the-features-of-the-trtllmattention-backend "Link to this heading")

The following sections introduce some features of the default `TrtllmAttention` backend.

### Packed Tensors[#](#packed-tensors "Link to this heading")
...
### Context and Generation Phases[#](#context-and-generation-phases "Link to this heading")
...
#### Context Phase[#](#context-phase "Link to this heading")
...
#### FP8 Context FMHA[#](#fp8-context-fmha "Link to this heading")
...
#### Generation Phase[#](#generation-phase "Link to this heading")
...
#### XQA Optimization[#](#xqa-optimization "Link to this heading")
...
### In-flight Batching[#](#in-flight-batching "Link to this heading")
...
*(1) Padding sequences in the generation phase, that contain a single token, to
...
### KV Cache[#](#kv-cache "Link to this heading")
...
#### Contiguous KV Cache[#](#contiguous-kv-cache "Link to this heading")
...
#### Paged KV Cache[#](#paged-kv-cache "Link to this heading")
...
[`KVCacheManager`](https://github.com/NVIDIA/TensorRT-LLM/tree/48b7b5d/tensorrt_llm/_torch/pyexecutor/resource_manager.py).

#### INT8/FP8 KV Caches[#](#int8-fp8-kv-caches "Link to this heading")
...
### Sliding Window Attention, Cyclic (Rolling Buffer) KV Cache[#](#sliding-window-attention-cyclic-rolling-buffer-kv-cache "Link to this heading")
...
### StreamingLLM[#](#streamingllm "Link to this heading")
...
That tensor is called the `cache_indirection`. Its shape is `[batch_size, beam_width, max_seqlen]`.
...
### Input QKV tensor[#](#input-qkv-tensor "Link to this heading")
...
### Rotary Positional Embedding (RoPE)[#](#rotary-positional-embedding-rope "Link to this heading")
...
### ALiBi[#](#alibi "Link to this heading")
...
### Scaling factor(s)[#](#scaling-factor-s "Link to this heading")
...
### Cross Attention[#](#cross-attention "Link to this heading")
...
Feature Combination Matrix](feature-combination-matrix.md "previous page")
[next

Disaggregated Serving (Beta)](disagg-serving.md "next page")

On this page

* [Attention Backends](#attention-backends)
* [Implement a New Attention Backend](#implement-a-new-attention-backend)
  + [Implement `AttentionMetadata`](#implement-attentionmetadata)
  + [Implement `AttentionBackend`](#implement-attentionbackend)
* [The Features of the `TrtllmAttention` Backend](#the-features-of-the-trtllmattention-backend)
  + [Packed Tensors](#packed-tensors)
  + [Context and Generation Phases](#context-and-generation-phases)
    - [Context Phase](#context-phase)
    - [FP8 Context FMHA](#fp8-context-fmha)
    - [Generation Phase](#generation-phase)
    - [XQA Optimization](#xqa-optimization)
  + [In-flight Batching](#in-flight-batching)
  + [Chunked Context](#chunked-context)
  + [KV Cache](#kv-cache)
    - [Contiguous KV Cache](#contiguous-kv-cache)
    - [Paged KV Cache](#paged-kv-cache)
    - [INT8/FP8 KV Caches](#int8-fp8-kv-caches)
  + [Sliding Window Attention, Cyclic (Rolling Buffer) KV Cache](#sliding-window-attention-cyclic-rolling-buffer-kv-cache)
  + [StreamingLLM](#streamingllm)
  + [Beam-Search](#beam-search)
  + [Input QKV tensor](#input-qkv-tensor)
  + [Rotary Positional Embedding (RoPE)](#rotary-positional-embedding-rope)
  + [ALiBi](#alibi)
  + [Scaling factor(s)](#scaling-factor-s)
