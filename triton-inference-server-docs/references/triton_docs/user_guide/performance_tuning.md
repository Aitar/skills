...
# Deploying your trained model using Triton[#](#deploying-your-trained-model-using-triton "Link to this heading")
...
1. Is my model compatible with Triton?
   ...
   * If your model falls under one of Tritonâs
     ...
     minimal model configuration can be inferred from the model using Tritonâs
...
2. Can I run inference on my served model?
   ...
   * If Perf Analyzer fails to send requests and it is unclear from the error
     ...
     framework directly. If you donât have your own script or tool to do so,
     ...
     TensorFlow 1.x.
   * The definition of âperforming wellâ is subject to change for each use
...
3. How can I improve my model performance?
   ...
   * To further understand the best model configuration you can provide to
     Triton for your use case, Tritonâs
   ...
   * Some parameters that can be tuned for a model may not be exposed to Model
     Analyzerâs automatic search since they donât apply to all models.
...
### Other Areas of Interest[#](#other-areas-of-interest "Link to this heading")
...
1. My model performs slowly when it is first loaded by Triton
   ...
   * Triton exposes the ability to run
     ...
     being marked âREADYâ for inference.
2. Why doesnât my model perform significantly faster on GPU?
...
## End-to-end Example[#](#end-to-end-example "Link to this heading")
...
Letâs take an ONNX model as our example since ONNX is designed to be a format
that can be [easily
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| densenet\_onnx\_config\_3 | 0 | Enabled | 4/GPU | 35.8 | 323.13 | 3695 | 58.6 |
| densenet\_onnx\_config\_2 | 0 | Enabled | 3/GPU | 59.575 | 295.82 | 3615 | 58.9 |
| densenet\_onnx\_config\_4 | 0 | Enabled | 5/GPU | 69.939 | 291.468 | 3966 | 58.2 |
| densenet\_onnx\_config\_default | 0 | Disabled | 1/GPU | 12.658 | 167.549 | 3116 | 51.3 |
...
So letâs extract that `config.pbtxt` and put it back in our model repository for future use.
...
to learn more about Tritonâs complete set of capabilities.
