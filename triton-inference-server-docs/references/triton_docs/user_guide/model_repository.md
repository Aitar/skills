...
**Is this your first time setting up a model repository?** Check out [these tutorials](../tutorials/Conceptual_Guide/Part_1-model_deployment/README.md#setting-up-the-model-repository) to begin your Triton journey!
...
## Repository Layout[#](#repository-layout "Link to this heading")
...
Each of the  sub-directories contains the repository information for the corresponding model.
...
Each  directory may include an optional sub-directory configs.
Within the configs directory there must be zero or more  with .pbtxt file extension. For more information about how the custom model configuration is handled by Triton see [Custom Model Configuration](model_configuration.md#custom-model-configuration).

Each  directory must have at least one numeric sub-directory representing a version of the model.
...
## Model Repository Locations[#](#model-repository-locations "Link to this heading")
...
### Local File System[#](#local-file-system "Link to this heading")
...
### Cloud Storage with Environment variables[#](#cloud-storage-with-environment-variables "Link to this heading")

#### Google Cloud Storage[#](#google-cloud-storage "Link to this heading")
...
2. [The attached service account](https://cloud.google.com/docs/authentication/application-default-credentials#attached-sa)
   ...
   * A value for the [Authorization HTTP header](https://googleapis.dev/cpp/google-cloud-storage/1.42.0/classgoogle_1_1cloud_1_1storage_1_1oauth2_1_1ComputeEngineCredentials.md#a8c3a5d405366523e2f4df06554f0a676) should be obtainable.
3. Anonymous credential (also known as public bucket)
   ...
   * One way to grant such permission is by adding both [storage.objectViewer](https://cloud.google.com/storage/docs/access-control/iam-roles#standard-roles) and [storage.legacyBucketReader](https://cloud.google.com/storage/docs/access-control/iam-roles#legacy-roles) predefined roles for âallUsersâ to the bucket, for example:
...
When using S3, the credentials and default region can be passed by using either the [aws config](https://docs.aws.amazon.com/cli/latest/userguide/cli-chap-configure.md) command or via the respective [environment variables](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-envvars.md).
...
#### Azure Storage[#](#azure-storage "Link to this heading")
...
If you donât know your `AZURE_STORAGE_KEY` and have your Azure CLI correctly configured, hereâs an example of how to find a key corresponding to your `AZURE_STORAGE_ACCOUNT`:
...
### Cloud Storage with Credential file (Beta)[#](#cloud-storage-with-credential-file-beta "Link to this heading")
...
To match a credential, the longest matching credential name against the start of a given path is used. For example: `gs://gcs-bucket-002/model_repository` will match the âgs://gcs-bucket-002â GCS credential, and `gs://any-other-gcs-bucket` will match the ââ GCS credential.
...
### Caching of Cloud Storage[#](#caching-of-cloud-storage "Link to this heading")

Triton currently doesnât perform file caching for cloud storage.
...
## Model Versions[#](#model-versions "Link to this heading")
...
The subdirectories that are not numerically named, or have names that start with the character â0â will be ignored.
...
## Model Files[#](#model-files "Link to this heading")

The contents of each model version sub-directory is determined by the type of the model and the requirements of the [backend](../backend/README.md) that supports the model.

### TensorRT Models[#](#tensorrt-models "Link to this heading")

A TensorRT model definition is called a *Plan*. A TensorRT Plan is a single file that by default must be named model.plan.
This default name can be overridden using the *default\_model\_filename* property in the [model configuration](model_configuration.md).

A TensorRT Plan is specific to a GPUâs [CUDA Compute Capability](https://developer.nvidia.com/cuda-gpus).
As a result, TensorRT models will need to set the *cc\_model\_filenames* property in the [model configuration](model_configuration.md) to associate each Plan file with the corresponding Compute Capability.
...
### ONNX Models[#](#onnx-models "Link to this heading")
...
By default this directory must be named model.onnx but can be overridden using the *default\_model\_filename* property in the [model configuration](model_configuration.md).
...
### TorchScript Models[#](#torchscript-models "Link to this heading")
...
### OpenVINO Models[#](#openvino-models "Link to this heading")
...
### Python Models[#](#python-models "Link to this heading")
...
By default the Python script must be named model.py but this default name can be overridden using the *default\_model\_filename* property in the [model configuration](model_configuration.md).
...
### DALI Models[#](#dali-models "Link to this heading")
...
Please refer to [DALI backend documentation](../dali_backend/README.md#how-to-use) for the description, how to generate `model.dali`.
The default model file name can be overridden using the *default\_model\_filename* property in the [model configuration](model_configuration.md).
