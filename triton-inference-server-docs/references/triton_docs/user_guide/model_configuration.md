...
**Is this your first time writing a config file?** Check out [this guide](../tutorials/Conceptual_Guide/Part_1-model_deployment/README.md#model-configuration) or this [example](../tutorials/HuggingFace/README.md#examples)!
...
## Minimal Model Configuration[#](#minimal-model-configuration "Link to this heading")

A minimal model configuration must specify the [*platform* and/or *backend* properties](../backend/README.md#backends), the *max\_batch\_size* property, and the input and output tensors of the model.
...
### Name, Platform and Backend[#](#name-platform-and-backend "Link to this heading")
...
The required values for *platform* and *backend* are described in the [backend documentation](../backend/README.md#backends).

### Model Transaction Policy[#](#model-transaction-policy "Link to this heading")

The *model\_transaction\_policy* property describes the nature of transactions expected from the model.

#### Decoupled[#](#decoupled "Link to this heading")
...
The *max\_batch\_size* property indicates the maximum batch size that the model supports for the [types of batching](architecture.md#models-and-schedulers) that can be exploited by Triton.
If the modelâs batch dimension is the first dimension, and all inputs and outputs to the model have this batch dimension, then Triton can use its [dynamic batcher](#dynamic-batcher) or [sequence batcher](#sequence-batcher) to automatically use batching with the model.
In this case *max\_batch\_size* should be set to a value greater-or-equal-to 1 that indicates the maximum batch size that Triton should use with the model.

For models that do not support batching, or do not support batching in the specific ways described above, *max\_batch\_size* must be set to zero.
...
#### Special Conventions for PyTorch Backend[#](#special-conventions-for-pytorch-backend "Link to this heading")

**Naming Convention:**

Due to the absence of sufficient metadata for inputs/outputs in TorchScript model files, the ânameâ attribute of inputs/outputs in the configuration must follow specific naming conventions.
These are detailed below.

1. [Only for Inputs] When the input is not a Dictionary of Tensors, the input names in the configuration file should mirror the names of the input arguments to the forward function in the modelâs definition.

   For example, if the forward function for the Torchscript model was defined as `forward(self, input0, input1)`, the first and second inputs should be named âinput0â and âinput1â respectively.
2. `<name>__<index>`: Where <name> can be any string and <index> is an integer index that refers to the position of the corresponding input/output.
   ...
   This means that if there are two inputs and two outputs, the first and second inputs can be named âINPUT\_\_0â and âINPUT\_\_1â and the first and second outputs can be named âOUTPUT\_\_0â and âOUTPUT\_\_1â respectively.
...
Instead, the names of the inputs in this case must map to the string value âkeyâ for that specific tensor.
For this case, the inputs would be âAâ and âBâ, where input âAâ refers to value corresponding to tensor1 and âBâ refers to the value corresponding to tensor2.
...
Input and output shapes are specified by a combination of *max\_batch\_size* and the dimensions specified by the input or output *dims* property.
For models with *max\_batch\_size* greater-than 0, the full shape is formed as [ -1 ] + *dims*.
For models with *max\_batch\_size* equal to 0, the full shape is formed as *dims*.
For example, for the following configuration the shape of âinput0â is [-1, 16 ] and the shape of âoutput0â is [ -1, 4 ].
...
For a configuration that is identical except that *max\_batch\_size* equal to 0, the shape of âinput0â is [ 16 ] and the shape of âoutput0â is [ 4 ].
...
Triton would then accept inference requests where that input tensorâs second dimension was any value greater-or-equal-to 0.
...
In this case, Triton would only accept inference requests where the input tensorâs shape was exactly *[ 4, 4 ]*.
...
## Auto-Generated Model Configuration[#](#auto-generated-model-configuration "Link to this heading")
...
For Python models, [`auto_complete_config`](../python_backend/README.md#auto-complete-config) function can be implemented in Python backend to provide [`max_batch_size`](#maximum-batch-size), [`input`](#inputs-and-outputs) and [`output`](#inputs-and-outputs) properties using `set_max_batch_size`, `add_input`, and `add_output` functions.
...
Currently, only [inputs, outputs](#inputs-and-outputs), [max\_batch\_size](#maximum-batch-size) and [dynamic batching](#dynamic-batcher) settings can be populated by backend.
...
From this you can take the max\_batch\_size, inputs, and outputs sections of the JSON and convert it to a config.pbtxt file.
...
## Custom Model Configuration[#](#custom-model-configuration "Link to this heading")
...
Example 1: âmodel-config-name=h100
...
âââ model_repository/
    âââ model_a/
    â   âââ configs/
    â   â   âââ v100.pbtxt
    â   â   âââ **h100.pbtxt**
...
Example 2: âmodel-config-name=config
...
Example 3: âmodel-config-name not set
...
### Default Max Batch Size and Dynamic Batcher[#](#default-max-batch-size-and-dynamic-batcher "Link to this heading")
...
Backend developers may make use of this default-max-batch-size by obtaining it from the TRITONBACKEND\_BackendConfig api.
...
2. [TensorRT backend](https://github.com/triton-inference-server/tensorrt_backend)
   ...
   1. TensorRT models store the maximum batch size explicitly and do not make use of the default-max-batch-size parameter.
      However, if max\_batch\_size > 1 and no [scheduler](#scheduling-and-batching) is provided, the dynamic batch scheduler will be enabled.

If a value greater than 1 for the maximum batch size is set for the model, the [dynamic\_batching](#dynamic-batcher) config will be set if no scheduler is provided in the configuration file.
...
The sixth column, labeled âAPIâ, shows the corresponding datatype for the TRITONSERVER C API, TRITONBACKEND C API, HTTP/REST protocol and GRPC protocol.
...
| --- | --- | --- | --- | --- | --- |
| TYPE\_BOOL | kBOOL | BOOL | kBool | BOOL | bool |
| TYPE\_UINT8 | kUINT8 | UINT8 | kByte | UINT8 | uint8 |
| TYPE\_UINT16 |  | UINT16 |  | UINT16 | uint16 |
| TYPE\_UINT32 |  | UINT32 |  | UINT32 | uint32 |
| TYPE\_UINT64 |  | UINT64 |  | UINT64 | uint64 |
| TYPE\_INT8 | kINT8 | INT8 | kChar | INT8 | int8 |
| TYPE\_INT16 |  | INT16 | kShort | INT16 | int16 |
| TYPE\_INT32 | kINT32 | INT32 | kInt | INT32 | int32 |
| TYPE\_INT64 | kINT64 | INT64 | kLong | INT64 | int64 |
| TYPE\_FP16 | kHALF | FLOAT16 |  | FP16 | float16 |
| TYPE\_FP32 | kFLOAT | FLOAT | kFloat | FP32 | float32 |
| TYPE\_FP64 |  | DOUBLE | kDouble | FP64 | float64 |
| TYPE\_STRING |  | STRING |  | BYTES | dtype(object) |
| TYPE\_BF16 | kBF16 |  |  | BF16 |  |
...
For ONNX Runtime each value is prepended with ONNX\_TENSOR\_ELEMENT\_DATA\_TYPE\_.
For example, ONNX\_TENSOR\_ELEMENT\_DATA\_TYPE\_FLOAT is the 32-bit floating-point datatype.
...
## Reshape[#](#reshape "Link to this heading")
...
## Shape Tensors[#](#shape-tensors "Link to this heading")

For models that support shape tensors, the *is\_shape\_tensor* property must be set appropriately for inputs and outputs that are acting as shape tensors.
...
Triton requires the shape tensors to be marked as shape tensors in the model when using batching. Note that âinput1â has shape *[ 3 ]* and not *[ 2 ]*, which is how it is described in model configuration.
As `myshapetensormodel` model is a batching model, the batch size should be provided as an additional value.
Triton will accumulate all the shape values together for âinput1â in batch dimension before issuing the request to model.
...
Read [Shape Tensor I/O](https://docs.nvidia.com/deeplearning/tensorrt/developer-guide/index.md#shape_tensor_io) to learn more about shape tensors.

## Non-Linear I/O Formats[#](#non-linear-i-o-formats "Link to this heading")

For models that process input or output data in non-linear formats, the *is\_non\_linear\_format\_io* property must be set.
...
To learn more about I/O formats, refer to the [I/O Formats documentation](https://docs.nvidia.com/deeplearning/tensorrt/developer-guide/index.md#reformat-free-network-tensors).

## Version Policy[#](#version-policy "Link to this heading")
...
* *All*: All versions of the model that are available in the model repository are available for inferencing.
  `version_policy: { all: {}}`
* *Latest*: Only the latest ânâ versions of the model in the repository are available for inferencing.
...
## Instance Groups[#](#instance-groups "Link to this heading")
...
### Multiple Model Instances[#](#multiple-model-instances "Link to this heading")
...
For a more detailed example of using instance groups, see [this guide](../tutorials/Conceptual_Guide/Part_2-improving_resource_utilization/README.md#concurrent-model-execution).

### CPU Model Instance[#](#cpu-model-instance "Link to this heading")
...
If no `count` is specified for a KIND\_CPU instance group, then the default instance count will be 2 for selected backends (Onnxruntime).
...
The following configuration will associate all instances created by the instance group setting with host policy âpolicy\_0â.
By default the host policy will be set according to the device kind of the instance, for instance, KIND\_CPU is âcpuâ, KIND\_MODEL is âmodelâ, and KIND\_GPU is âgpu\_<gpu\_id>â.
...
### Rate Limiter Configuration[#](#rate-limiter-configuration "Link to this heading")
...
If rate limiting is on and if an instance\_group does not provide this configuration, then the execution on the model instances belonging to this group will not be limited in any way by the rate limiter.
...
The ânameâ field identifies the resource and âcountâ field refers to the number of copies of the resource that the model instance in the group requires to run.
The âglobalâ field specifies whether the resource is per-device or shared globally across the system.
...
The following example specifies the instances in the group requires four âR1â and two âR2â resources for execution.
Resource âR2â is a global resource.
Additionally, the rate-limiter priority of the instance\_group is 2.
...
The three instances will not contend for âR1â among themselves as âR1â is local for their own device, however, they will contend for âR2â because it is specified as a global resource which means âR2â is shared across the system.
Though these instances donât contend for âR1â among themselves, but they will contend for âR1â with other model instances which includes âR1â in their resource requirements and run on the same device as them.

### Ensemble Model Instance Groups[#](#ensemble-model-instance-groups "Link to this heading")
...
## CUDA Compute Capability[#](#cuda-compute-capability "Link to this heading")

Similar to the `default_model_filename` field, you can optionally specify the `cc_model_filenames` field to map the GPUâs [CUDA Compute Capability](https://developer.nvidia.com/cuda-gpus) to a corresponding model filename at model load time.
...
## Optimization Policy[#](#optimization-policy "Link to this heading")
...
## Model Warmup[#](#model-warmup "Link to this heading")

When a model is loaded by Triton the corresponding [backend](../backend/README.md) initializes for that model.
...
To avoid these initial, slow inference requests, Triton provides a configuration option that enables a model to be âwarmed upâ so that it is completely initialized before the first inference request is received.
...
See the [ModelWarmup protobuf](https://github.com/triton-inference-server/common/blob/main/protobuf/model_config.proto) documentation for the currently available settings, and [L0\_warmup](https://github.com/triton-inference-server/server/blob/main/qa/L0_warmup/test.sh) for examples on specifying different variants of warmup samples.

## Response Cache[#](#response-cache "Link to this heading")
