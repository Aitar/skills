...
# Python Backend[#](#python-backend "Link to this heading")
...
* [Python Backend](#python-backend)
  ...
  + [Usage](#usage)
    ...
    - [`auto_complete_config`](#auto-complete-config)
...
* [Interoperability and GPU Support](#interoperability-and-gpu-support)
  ...
  + [`pb_utils.Tensor.to_dlpack() -> PyCapsule`](#pb-utils-tensor-to-dlpack-pycapsule)
  + [`pb_utils.Tensor.from_dlpack() -> Tensor`](#pb-utils-tensor-from-dlpack-tensor)
  + [`pb_utils.Tensor.is_cpu() -> bool`](#pb-utils-tensor-is-cpu-bool)
...
the build. If the CMake variables below are not specified, âmainâ branch
...
### `auto_complete_config`[#](#auto-complete-config "Link to this heading")

`auto_complete_config` is called only once when loading the model assuming
the server was not started with
[`--disable-auto-complete-config`](../user_guide/model_configuration.md#auto-generated-model-configuration).
...
[dynamic\_batching](../user_guide/model_configuration.md#dynamic-batcher),
[`input`](../user_guide/model_configuration.md#inputs-and-outputs)
and
[`output`](../user_guide/model_configuration.md#inputs-and-outputs)
...
In addition to minimal properties, you can also set [model\_transaction\_policy](../user_guide/model_configuration.md#model-transaction-policy)
...
### `initialize`[#](#initialize "Link to this heading")
...
#### Default Mode[#](#default-mode "Link to this heading")
...
#### Request Cancellation Handling[#](#request-cancellation-handling "Link to this heading")
...
#### Decoupled mode[#](#decoupled-mode "Link to this heading")
...
[transaction policy](../user_guide/model_configuration.md#model-transaction-policy)
...
##### Async Execute[#](#async-execute "Link to this heading")
...
#### Request Rescheduling[#](#request-rescheduling "Link to this heading")
...
takes 1 input tensor, an INT32 [ 1 ] input named âINâ, and produces an output
tensor âOUTâ with the same shape as the input tensor. The input value indicates
...
### `finalize`[#](#finalize "Link to this heading")
...
## Model Config File[#](#model-config-file "Link to this heading")
...
shouldnât set `platform` field of the configuration.
...
## Inference Request Parameters[#](#inference-request-parameters "Link to this heading")
...
extension](../protocol/extension_parameters.md)
documentation.

## Inference Response Parameters[#](#inference-response-parameters "Link to this heading")
...
## Managing Python Runtime and Libraries[#](#managing-python-runtime-and-libraries "Link to this heading")
...
the Python version of the Python backendâs stub executable. For example,
...
### Building Custom Python Backend Stub[#](#building-custom-python-backend-stub "Link to this heading")
...
`conda activate <conda-env-name>`. Note that you donât have to use `conda` and
...
[pybind11 decides which Python to use](https://pybind11.readthedocs.io/en/stable/faq.md?highlight=cmake#cmake-doesn-t-detect-the-right-python-version).
...
### Creating Custom Execution Environments[#](#creating-custom-execution-environments "Link to this heading")
...
[`PYTHONNOUSERSITE`](https://docs.python.org/3/using/cmdline.md#envvar-PYTHONNOUSERSITE)
...
using [conda create -p](https://docs.conda.io/projects/conda/en/latest/commands/create.md).
...
### Important Notes[#](#important-notes "Link to this heading")
...
2. If you donât want to use a different Python interpreter, you can skip
   [Building Custom Python Backend Stub](#building-custom-python-backend-stub).
...
7. If you encounter the âGLIBCXX\_3.4.30 not foundâ error during runtime, we
   ...
   encounter the âGLIBCXX\_3.4.32 not foundâ error during runtime, the recommended
...
## Error Handling[#](#id1 "Link to this heading")
...
## Managing Shared Memory[#](#managing-shared-memory "Link to this heading")

Starting from 21.04 release, Python backend uses shared memory to connect
userâs code to Triton. Note that this change is completely transparent and
does not require any change to the existing userâs model code.
...
## Multiple Model Instance Support[#](#multiple-model-instance-support "Link to this heading")

Python interpreter uses a global lock known as
[GIL](https://docs.python.org/3/c-api/init.md#thread-state-and-the-global-interpreter-lock).
...
[model instance](../user_guide/model_configuration.md#multiple-model-instances).
...
## Running Multiple Instances of Triton Server[#](#running-multiple-instances-of-triton-server "Link to this heading")
...
If youâre using a Python backend released before the 24.04 release, you need
...
donât share this location, you donât need to specify `shm-region-prefix-name`.

# Business Logic Scripting[#](#business-logic-scripting "Link to this heading")

Tritonâs
[ensemble](../user_guide/architecture.md#ensemble-models)
  ...
  ...
    ...
    async def execute(self, requests):
      ...
      for i in range(4):
        ...
        # [Awaitable](https://docs.python.org/3/library/asyncio-task.md#awaitables)
        # object.
...
## Using BLS with Decoupled Models[#](#using-bls-with-decoupled-models "Link to this heading")
...
[iterator](https://docs.python.org/3/glossary.md#term-iterator) of
...
the parameter âtimeoutâ in microseconds within the constructor of
`InferenceRequest`. If the request times out, the request will respond with an
error. The default of âtimeoutâ is 0 which indicates that the request has no
timeout.
...
### Cancelling decoupled BLS requests[#](#cancelling-decoupled-bls-requests "Link to this heading")
...
the request depends on the modelâs backend implementation. Please refer to the
documentation for more details [Handing in Backend](../user_guide/request_cancellation.md#handling-in-backend)

## Model Loading API[#](#model-loading-api "Link to this heading")
...
[explicit model control mode](../user_guide/model_management.md#model-control-mode-explicit).
...
## Using BLS with Stateful Models[#](#using-bls-with-stateful-models "Link to this heading")
...
# Interoperability and GPU Support[#](#interoperability-and-gpu-support "Link to this heading")
...
## `pb_utils.Tensor.to_dlpack() -> PyCapsule`[#](#pb-utils-tensor-to-dlpack-pycapsule "Link to this heading")
...
## `pb_utils.Tensor.from_dlpack() -> Tensor`[#](#pb-utils-tensor-from-dlpack-tensor "Link to this heading")
...
[`__dlpack__`](https://data-apis.org/array-api/2022.12/API_specification/generated/array_api.array.__dlpack__.md)
and [`__dlpack_device__`](https://data-apis.org/array-api/2022.12/API_specification/generated/array_api.array.__dlpack_device__.md)
[interface](https://dmlc.github.io/dlpack/latest/python_spec.md)
...
## `pb_utils.Tensor.is_cpu() -> bool`[#](#pb-utils-tensor-is-cpu-bool "Link to this heading")

This function can be used to check whether a tensor is placed in CPU or not.

## Input Tensor Device Placement[#](#input-tensor-device-placement "Link to this heading")
...
behavior. By setting `FORCE_CPU_ONLY_INPUT_TENSORS` to ânoâ, Triton will not
...
### PyTorch Determinism[#](#pytorch-determinism "Link to this heading")
...
For most intents and purposes, these differences arenât large enough to affect
a modelâs final prediction. However, to understand where these differences come
from, see this [doc](https://pytorch.org/docs/stable/notes/randomness.md).
...
it as needed, see
[here](https://pytorch.org/docs/stable/notes/cuda.md#tensorfloat-32-tf32-on-ampere-devices).

## TensorFlow[#](#tensorflow "Link to this heading")

### TensorFlow Determinism[#](#tensorflow-determinism "Link to this heading")
...
configurations, or batch sizes due to the libraryâs internal CUDA kernel
...
# Custom Metrics[#](#custom-metrics "Link to this heading")
...
[TRITON C API custom metrics](../user_guide/metrics.md#custom-metrics)
...
deleted if youâd like to explicitly delete the custom metrics objects.
...
the
[Triton Python Client Library](../client/README.md#getting-the-client-libraries-and-examples).
The Python client for each of the examples is in the `client.py` file.

## AddSub in NumPy[#](#addsub-in-numpy "Link to this heading")
...
## AddSubNet in PyTorch[#](#addsubnet-in-pytorch "Link to this heading")
...
## AddSub in JAX[#](#addsub-in-jax "Link to this heading")
...
## Decoupled Models[#](#decoupled-models "Link to this heading")
...
## Model Instance Kind[#](#model-instance-kind "Link to this heading")
...
## Auto-complete config[#](#id3 "Link to this heading")
...
instructions in [examples/auto\_complete](examples/auto_complete/README.md).

## Custom Metrics[#](#id4 "Link to this heading")

The example shows how to use custom metrics API in Python Backend. You can find
the complete example instructions in
[examples/custom\_metrics](examples/custom_metrics/README.md).

# Running with Inferentia[#](#running-with-inferentia "Link to this heading")

Please see the
[README.md](inferentia/README.md)
...
Note that the Triton serverâs settings determine which log messages appear
...
appear in the server log. For more information on Tritonâs log settings and
how to adjust them dynamically, please see Tritonâs
[logging extension](../protocol/extension_logging.md)
documentation.

# Adding Custom Parameters in the Model Configuration[#](#adding-custom-parameters-in-the-model-configuration "Link to this heading")
...
# Development with VSCode[#](#development-with-vscode "Link to this heading")
...
In order to build the backend, you can execute the âBuild Python Backendâ task in the
...
* minimal â use as little code as possible that still produces the
  same problem
* complete â provide all parts needed to reproduce the problem. Check
  if you can strip external dependency and still show the problem. The
  less time we spend on reproducing problems the more time we have to
  fix it
* verifiable â test the code youâre about to provide to make sure it
