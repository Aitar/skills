...
# Using Triton with Inferentia 1[#](#using-triton-with-inferentia-1 "Link to this heading")

Starting from 21.11 release, Triton supports
[AWS Inferentia](https://aws.amazon.com/machine-learning/inferentia/)
and the [Neuron Runtime](https://awsdocs-neuron.readthedocs-hosted.com/en/latest/neuron-intro/get-started.md).
...
* [Using Triton with Inferentia 1](#using-triton-with-inferentia-1)
  ...
  + [Serving Inferentia model in Triton](#serving-inferentia-model-in-triton)
    ...
    - [Using Tritonâs Dynamic Batching](#using-triton-s-dynamic-batching)
  + [Testing Inferentia Setup for Accuracy](#testing-inferentia-setup-for-accuracy)

## Inferentia setup[#](#inferentia-setup "Link to this heading")
...
[Triton Inference Server Container Release Notes](https://docs.nvidia.com/deeplearning/triton-inference-server/release-notes/index.md).
...
2. Install [neuron-cc](https://awsdocs-neuron.readthedocs-hosted.com/en/latest/neuron-guide/neuron-cc/index.md), the Neuron compiler.
...
## Setting up the Inferentia model[#](#setting-up-the-inferentia-model "Link to this heading")

Currently, we only support [PyTorch](https://awsdocs-neuron.readthedocs-hosted.com/en/latest/neuron-guide/neuron-frameworks/pytorch-neuron/index.md)
and [TensorFlow](https://awsdocs-neuron.readthedocs-hosted.com/en/latest/neuron-guide/neuron-frameworks/tensorflow-neuron/index.md)
...
* [Neuron compiler CLI Reference Guide](https://awsdocs-neuron.readthedocs-hosted.com/en/latest/neuron-guide/neuron-cc/command-line-reference.md)
* [PyTorch-Neuron trace python API](https://awsdocs-neuron.readthedocs-hosted.com/en/latest/neuron-guide/neuron-frameworks/pytorch-neuron/api-compilation-python-api.md)
* [PyTorch Tutorials](https://awsdocs-neuron.readthedocs-hosted.com/en/latest/neuron-guide/neuron-frameworks/pytorch-neuron/tutorials/index.md)
* [TensorFlow Tutorials](https://awsdocs-neuron.readthedocs-hosted.com/en/latest/neuron-guide/neuron-frameworks/tensorflow-neuron/tutorials/index.md)
  for guidance on how to compile models.

### PyTorch[#](#pytorch "Link to this heading")

For PyTorch, we support models traced by [PyTorch-Neuron trace python API](https://awsdocs-neuron.readthedocs-hosted.com/en/latest/neuron-guide/neuron-frameworks/pytorch-neuron/api-compilation-python-api.md)
for execution on Inferentia.
Once the TorchScript model supporting Inferentia is obtained, use the
[gen\_triton\_model.py](https://github.com/triton-inference-server/python_backend/blob/main/inferentia/scripts/gen_triton_model.py) script to generate
...
as: âINPUT\_\_0â, âINPUT\_\_1â and âOUTPUT\_\_0â, âOUTPUT\_\_1â such
that âINPUT\_\_0â refers to first input and INPUT\_\_1 refers to the
second input, etc.
...
[Data Parallel Inference](https://awsdocs-neuron.readthedocs-hosted.com/en/latest/neuron-guide/appnotes/perf/torch-neuron-dataparallel-app-note.md)
...
[AWS Neuron TensorFlow](https://awsdocs-neuron.readthedocs-hosted.com/en/latest/neuron-guide/neuron-frameworks/tensorflow-neuron/tutorials/index.md)
...
Once the compiled model is obtained use [gen\_triton\_model.py](https://github.com/triton-inference-server/python_backend/blob/main/inferentia/scripts/gen_triton_model.py)
...
instances. However, the neuron core indices donât point to a specific
...
[Parallel Execution using NEURONCORE\_GROUP\_SIZES](https://awsdocs-neuron.readthedocs-hosted.com/en/latest/neuron-guide/appnotes/perf/parallel-ncgs.md?highlight=NEURONCORE_GROUP_SIZES)
...
## Serving Inferentia model in Triton[#](#serving-inferentia-model-in-triton "Link to this heading")
...
name. Move that model directory to Tritonâs model repository.
...
### Using Tritonâs Dynamic Batching[#](#using-triton-s-dynamic-batching "Link to this heading")
...
options for configuring [Tritonâs dynamic batching](../../user_guide/model_configuration.md):

1. `--preferred_batch_size`: Please refer to [model configuration documentation](../../user_guide/model_configuration.md#preferred-batch-sizes) for details on preferred batch size. To optimize
...
2. `--max_queue_delay_microseconds`: Please refer to
   [model configuration documentation](../../user_guide/model_configuration.md#delayed-batching) for details.
...
refer to the
[model configuration documentation](../../user_guide/model_configuration.md#maximum-batch-size)
for details.

## Testing Inferentia Setup for Accuracy[#](#testing-inferentia-setup-for-accuracy "Link to this heading")

The [qa folder](https://github.com/triton-inference-server/python_backend/tree/main/inferentia/qa)
contains the necessary files to set up testing with a simple add\_sub model. The test
...
# Using Triton with Inferentia 2, or Trn1[#](#using-triton-with-inferentia-2-or-trn1 "Link to this heading")

## pytorch-neuronx and tensorflow-neuronx[#](#pytorch-neuronx-and-tensorflow-neuronx "Link to this heading")
...
## transformers-neuronx[#](#transformers-neuronx "Link to this heading")

To use inf2/trn1 instances with transformers-neuronx packages for serving models, generate a `pytorch` model as per above instructions. The transformers-neuronx currently supports the models listed [here](https://awsdocs-neuron.readthedocs-hosted.com/en/latest/frameworks/torch/torch-neuronx/transformers-neuronx/readme.md#currently-supported-models).

As prescribed on the neuronx documentation page, while the neuronx load API differs per model, it follows the same pattern.

1. To serve transformers-neuronx models, first trace the model using `save_pretrained_split()` API on an inf2 instance (recommend inf2.24xl for Large Language Models). Following that, package the folder as the ââcompiled\_modelâ when using `gen_triton_model.py` file.
...
âââ opt-125m-model
âÂ Â  âââ pytorch_model.bin
âââ opt-125m-tp12
    âââ FullyUnrolled.1814.1
    âÂ Â  âââ penguin-sg0000
    âÂ Â  âââ sg00
    âââ FullyUnrolled.1814.2
    âÂ Â  âââ penguin-sg0000
    âÂ Â  âââ sg00
    âââ FullyUnrolled.1814.3
    âÂ Â  âââ penguin-sg0000
    âÂ Â  âââ sg00
    âââ FullyUnrolled.1814.4
    âÂ Â  âââ penguin-sg0000
    âÂ Â  âââ sg00
    âââ FullyUnrolled.1814.5
        âââ penguin-sg0000
        âââ sg00
  âââ config.pbtxt
```

3. Add the following imports (e.g., for OPT model). The import will differ as per the model youâre trying to run.
