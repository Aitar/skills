...
# Deploying Llama2-7B Model with Triton and vLLM[#](#deploying-llama2-7b-model-with-triton-and-vllm "Link to this heading")

The vLLM Backend uses vLLM to do inference. Read more about vLLM [here](https://blog.vllm.ai/2023/06/20/vllm.md) and the vLLM Backend [here](https://github.com/triton-inference-server/vllm_backend).
...
## Sending requests via the `generate` endpoint[#](#sending-requests-via-the-generate-endpoint "Link to this heading")

As a simple example to make sure the server works, you can use the `generate` endpoint to test. More about the generate endpoint [here](../../../protocol/extension_generate.md).
...
## Sending requests via the Triton client[#](#sending-requests-via-the-triton-client "Link to this heading")
