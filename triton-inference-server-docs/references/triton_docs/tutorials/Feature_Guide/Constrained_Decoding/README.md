...
# Constrained Decoding with Triton Inference Server[#](#constrained-decoding-with-triton-inference-server "Link to this heading")
...
## Introduction to Constrained Decoding[#](#introduction-to-constrained-decoding "Link to this heading")
...
In this tutorial weâll show how to use *LM Format Enforcer* and *Outlines*
in your workflow.

## Prerequisite: Hermes-2-Pro-Llama-3-8B[#](#prerequisite-hermes-2-pro-llama-3-8b "Link to this heading")

Before proceeding, please make sure that youâve successfully deployed
...
## Structured Generation via Prompt Engineering[#](#structured-generation-via-prompt-engineering "Link to this heading")
...
Please, refer to [`client.py`](https://github.com/triton-inference-server/tutorials/blob/main/Feature_Guide/Constrained_Decoding/artifacts/client.py) for full `prompt`
composition logic.
...
in [`client.py`](https://github.com/triton-inference-server/tutorials/blob/main/Feature_Guide/Constrained_Decoding/artifacts/client.py) we use a `pydantic` library to define the
...
## Enforcing Output Format via External Libraries[#](#enforcing-output-format-via-external-libraries "Link to this heading")

In this section of the tutorial, weâll show how to impose constrains on LLMs,
which are not inherently fine-tuned for constrained decoding. Weâll
[*LM Format Enforcer*](https://github.com/noamgat/lm-format-enforcer?tab=readme-ov-file)
and [*Outlines*](https://github.com/outlines-dev/outlines?tab=readme-ov-file)
offer robust solutions.

The reference implementation for both libraries is provided in
[`utils.py`](https://github.com/triton-inference-server/tutorials/blob/main/Feature_Guide/Constrained_Decoding/artifacts/utils.py) script, which also defines the output
format `AnswerFormat`:
...
### Pre-requisite: Common set-up[#](#pre-requisite-common-set-up "Link to this heading")

Make sure youâve successfully deployed Hermes-2-Pro-Llama-3-8B model
...
Weâll do some adjusments to model files, thu if you have a running server, you
can stop it via:
...
#### Logits Post-Processor[#](#logits-post-processor "Link to this heading")

Both of the libraries limit the set of allowed tokens at every generation stage.
In TensorRT-LLM, user can define a custom
[logits post-processor](https://nvidia.github.io/TensorRT-LLM/advanced/batch-manager.md#logits-post-processor-optional)
...
is set to `python` in `tensorrt_llm/config.pbtxt`, Tritonâs python backend will
use
...
during modelâs initialization as a part of
[Executorâs](https://nvidia.github.io/TensorRT-LLM/executor.md#executor-api)
configuration
...
In this tutorial, weâre deploying Hermes-2-Pro-Llama-3-8B model as a part of an
...
#### Tokenizer[#](#tokenizer "Link to this heading")
...
weâll be exposing tokenizer via `inflight_batcher_llm/tensorrt_llm/config.pbtxt`
parameter:
...
#### Repository set up[#](#repository-set-up "Link to this heading")

Weâve provided a sample implementation for *LM Format Enforcer* and *Outlines*
in [`artifacts/utils.py`](https://github.com/triton-inference-server/tutorials/blob/main/Feature_Guide/Constrained_Decoding/artifacts/utils.py). Make sure youâve copied it into
...
Finally, letâs install all required libraries:

```
pip install pydantic lm-format-enforcer outlines setuptools
```

### LM Format Enforcer[#](#lm-format-enforcer "Link to this heading")
...
##### Option 1. Use provided [client script](https://github.com/triton-inference-server/tutorials/blob/main/Feature_Guide/Constrained_Decoding/artifacts/client.py)[#](#option-1-use-provided-client-script "Link to this heading")

Letâs first send a standard request, without enforcing the JSON answer format:
...
Now, letâs specify `logits_post_processor_name` in our request:
...
As we can see, the schema, defined in [`utils.py`](https://github.com/triton-inference-server/tutorials/blob/main/Feature_Guide/Constrained_Decoding/artifacts/utils.py) is
respected. Note, LM Format Enforcer lets LLM to control the order of generated
fields, thus re-ordering of fields is allowed.

##### Option 2. Use [generate endpoint](https://github.com/triton-inference-server/tensorrtllm_backend/tree/release/0.5.0#query-the-server-with-the-triton-generate-endpoint).[#](#option-2-use-generate-endpoint "Link to this heading")
...
### Outlines[#](#outlines "Link to this heading")
...
#### Send an inference request[#](#id1 "Link to this heading")
...
##### Option 1. Use provided [client script](https://github.com/triton-inference-server/tutorials/blob/main/Feature_Guide/Constrained_Decoding/artifacts/client.py)[#](#id2 "Link to this heading")
...
##### Option 2. Use [generate endpoint](https://github.com/triton-inference-server/tensorrtllm_backend/tree/release/0.5.0#query-the-server-with-the-triton-generate-endpoint).[#](#id3 "Link to this heading")
