...
model on the Triton Inference Server using Tritonâs [Python backend](https://github.com/triton-inference-server/python_backend).
...
## Step 1: Create a Model Repository[#](#step-1-create-a-model-repository "Link to this heading")
...
* **config.pbtxt** - Outlines the backend to use, model input/output details, and custom
  ...
  properties Triton supports can be found [here](https://docs.nvidia.com/deeplearning/triton-inference-server/user-guide/docs/user_guide/model_configuration.md).
* **model.py** - Implements how Triton should handle the model during the initialization,
  ...
  can be found [here](../../../python_backend/README.md#usage).

## Step 2: Build a Triton Container Image[#](#step-2-build-a-triton-container-image "Link to this heading")
...
## Step 3: Launch the Triton Inference Server[#](#step-3-launch-the-triton-inference-server "Link to this heading")
...
## Step 4: Query the Server[#](#step-4-query-the-server "Link to this heading")
...
## Step 5: Host Multiple Models in Triton[#](#step-5-host-multiple-models-in-triton "Link to this heading")
...
by Triton in a simplified fashion by using Tritonâs generate endpoint:

```
curl -X POST localhost:8000/v2/models/falcon7b/generate -d '{"text_input":"How can you be"}'
```

## âDay Zeroâ Support[#](#day-zero-support "Link to this heading")
...
load these âbleeding edgeâ models in Triton by building `transformers` from source.
...
### Loading Cached Models[#](#loading-cached-models "Link to this heading")
...
your `model.py` **before** importing the âtransformersâ module, making sure to replace
...
command from earlier (making sure to replace `${HOME}` with the path to your associated usernameâs home directory):
...
### Triton Tool Ecosystem[#](#triton-tool-ecosystem "Link to this heading")
...
* [Performance Analyzer](https://docs.nvidia.com/deeplearning/triton-inference-server/archives/triton-inference-server-2310/user-guide/docs/user_guide/perf_analyzer.md): An inference performance optimizer.
* [Model Analyzer](https://docs.nvidia.com/deeplearning/triton-inference-server/user-guide/docs/user_guide/model_analyzer.md) A GPU memory and compute utilization optimizer.
...
This container comes with all of Tritonâs deployment analyzers pre-installed, meaning
we can simply enter the following to get feedback on our modelâs inference performance:
...
as adding the following to falconâs `config.pbtxt` file:
...
for this example, we select 8. Now letâs re-run the perf\_analyzer with increasing levels
...
[this](https://docs.nvidia.com/deeplearning/triton-inference-server/archives/triton-inference-server-2310/user-guide/docs/user_guide/perf_analyzer.md)
guide.

For more information regarding dynamic batching in Triton, please see [this](https://docs.nvidia.com/deeplearning/triton-inference-server/user-guide/docs/user_guide/model_configuration.md#dynamic-batcher)
guide.
...
To sweep this parameter space in a more robust fashion, we can use Tritonâs model analyzer, which
...
it with the following command (ensuring the dynamic\_batching parameters from above have been added
to the falcon modelâs config.pbtxt):
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| falcon7b\_config\_7 | 16 | Enabled | 3:GPU | 1412.581 | 71.944 | 46226 | 100.0 |
| falcon7b\_config\_8 | 32 | Enabled | 3:GPU | 2836.225 | 63.9652 | 46268 | 100.0 |
| falcon7b\_config\_4 | 16 | Enabled | 2:GPU | 7601.437 | 63.9454 | 31331 | 100.0 |
| falcon7b\_config\_default | 8 | Enabled | 1:GPU | 4151.873 | 63.9384 | 16449 | 89.3 |

We can examine the performance of any of these configurations with more granularity by viewing
their detailed reports. This subset of reports focuses on a single configurationâs latency
...
of model analyzer parameters and run options, please see [this](https://docs.nvidia.com/deeplearning/triton-inference-server/user-guide/docs/user_guide/model_analyzer.md) guide.
...
For a more custom deployment, please see our [model configuration guide](https://docs.nvidia.com/deeplearning/triton-inference-server/user-guide/docs/user_guide/model_configuration.md) to see how the scope of this tutorial can be expanded to fit your needs.
