...
# Autoscaling and Load Balancing Generative AI w/ Triton Server and TensorRT-LLM[#](#autoscaling-and-load-balancing-generative-ai-w-triton-server-and-tensorrt-llm "Link to this heading")
...
TensorRT, and configuring automatic scaling and load balancing for your models. This guide does not cover Kubernetesâ
...
## Cluster Setup[#](#cluster-setup "Link to this heading")
...
### Core Cluster Services[#](#core-cluster-services "Link to this heading")
...
#### Kubernetes Node Feature Discovery service[#](#kubernetes-node-feature-discovery-service "Link to this heading")
...
2. Run the command below to install the service.
   ...
   > a matching taint.
   > See this documentâs [prerequisites](#prerequisites) for the taints this document expected to have been applied to GPU
   > nodes in the cluster.

#### NVIDIA Device Plugin for Kubernetes[#](#nvidia-device-plugin-for-kubernetes "Link to this heading")
...
#### NVIDIA GPU Feature Discovery Service[#](#nvidia-gpu-feature-discovery-service "Link to this heading")
...
2. If `gpu-feature-discover` is listed, skip this step and the next.
   ...
   > [nvidia\_gpu-feature-discovery\_daemonset.yaml](https://github.com/triton-inference-server/tutorials/blob/main/Deployment/Kubernetes/TensorRT-LLM_Autoscaling_and_Load_Balancing/nvidia_gpu-feature-discovery_daemonset.yaml)
...
### Metrics Collection Services[#](#metrics-collection-services "Link to this heading")
...
#### Create a Monitoring Namespace[#](#create-a-monitoring-namespace "Link to this heading")
...
#### Prometheus Services[#](#prometheus-services "Link to this heading")
...
Using the following steps, weâll install the Prometheus Stack for Kubernetes Helm chart so that we can leverage Prometheus.
...
#### NVIDIA Data Center GPU Manager (DCGM) Exporter[#](#nvidia-data-center-gpu-manager-dcgm-exporter "Link to this heading")
...
Instead, weâll use the steps below to install just the [DCGM Exporter](https://github.com/NVIDIA/dcgm-exporter) to enable the
...
2. Use the YAML file below to install the DCGM Exporter.
   ...
   > [nvidia\_dcgm-exporter\_values.yaml](https://github.com/triton-inference-server/tutorials/blob/main/Deployment/Kubernetes/TensorRT-LLM_Autoscaling_and_Load_Balancing/nvidia_dcgm-exporter_values.yaml)
...
#### Connect DCGM and Triton Metrics to Prometheus[#](#connect-dcgm-and-triton-metrics-to-prometheus "Link to this heading")

We need to provide a mechanism that will export the metrics collected by Prometheus Server and make them available to
Kubernetesâ Horizontal Pod Autoscaler service.
...
#### Triton Metrics Prometheus Rule[#](#triton-metrics-prometheus-rule "Link to this heading")
...
Prometheus.
Weâll create a set of rules specific to Triton Server which generate metrics useful for autoscaling.

1. Use the YAML contents below to create a file named `triton-metrics_prometheus-rule.yaml`.

   > [triton-metrics\_prometheus-rule.yaml](https://github.com/triton-inference-server/tutorials/blob/main/Deployment/Kubernetes/TensorRT-LLM_Autoscaling_and_Load_Balancing/triton-metrics_prometheus-rule.yaml)
2. Run the following command to create the necessary Prometheus Rule in the cluster. *Note* that the rule will be created
...
### NFS Creation[#](#nfs-creation "Link to this heading")
...
To enable multiple pods deployed to multiple nodes to load shards of the same model so that they can used in coordination to serve inference request too large to loaded by a single GPU, weâll need a common, shared storage location. In Kubernetes, these common, shared storage locations are referred to as persistent volumes. Persistent volumes can be volume mapped in to any number of pods and then accessed by processes running inside of said pods as if they were part of the podâs file system. We will be using EFS as persistent volume.

Additionally, we will need to create a persistent-volume claim which can use to assign the persistent volume to a pod.

#### 1. Create an IAM role[#](#create-an-iam-role "Link to this heading")

Follow the steps to create an IAM role for your EFS file system: https://docs.aws.amazon.com/eks/latest/userguide/efs-csi.md#efs-create-iam-resources. This role will be used later when you install the EFS CSI Driver.

#### 2. Install EFS CSI driver[#](#install-efs-csi-driver "Link to this heading")

Install the EFS CSI Driver through the Amazon EKS add-on in AWS console: https://docs.aws.amazon.com/eks/latest/userguide/efs-csi.md#efs-install-driver. Once itâs done, check the Add-ons section in EKS console, you should see the driver is showing `Active` under Status.

#### 3. Create EFS file system[#](#create-efs-file-system "Link to this heading")
...
#### 4. Test NFS[#](#test-nfs "Link to this heading")

Follow the steps to check if your EFS file system is working properly with your nodes: https://github.com/kubernetes-sigs/aws-efs-csi-driver/tree/master/examples/kubernetes/multiple\_pods. This test is going to mount your EFS file system on all of your available nodes and write a text file to the file system.

#### 5. Create an PVC for the created EFS file system[#](#create-an-pvc-for-the-created-efs-file-system "Link to this heading")

We have provided an example in here: [pvc\_aws](https://github.com/triton-inference-server/tutorials/blob/main/Deployment/Kubernetes/TensorRT-LLM_Autoscaling_and_Load_Balancing/pvc_aws). This folder contains three files: `pv_aws.yaml`, `claim_aws.yaml`, and `storageclass_aws.yaml`. Make sure you modify the `pv_aws.yaml` file and change the `volumeHandle` value to your own EFS file system ID.
...
#### 6. Edit cliamName in deployment.yaml to point to your chart/templates/deployment.yaml[#](#edit-cliamname-in-deployment-yaml-to-point-to-your-chart-templates-deployment-yaml "Link to this heading")
...
## Triton Preparation[#](#triton-preparation "Link to this heading")

### Pod Initialization Script[#](#pod-initialization-script "Link to this heading")

1. Create a Python file with the content below named `server.py`.

   > [server.py](https://github.com/triton-inference-server/tutorials/blob/main/Deployment/Kubernetes/TensorRT-LLM_Autoscaling_and_Load_Balancing/containers/server.py)
...
### Model Preparation Steps:[#](#model-preparation-steps "Link to this heading")
...
1. Modify the `setup_ssh_nfs.yaml` file
   ...
   We use the `setup_ssh_nfs.yaml` file which does âsleep infinityâ to set up ssh access inside the compute node along with EFS.
...
#### Custom Container Image[#](#custom-container-image "Link to this heading")

1. Using the file below, weâll create a custom container image in the next step.

   > [triton\_trt-llm.containerfile](https://github.com/triton-inference-server/tutorials/blob/main/Deployment/Kubernetes/TensorRT-LLM_Autoscaling_and_Load_Balancing/containers/triton_trt-llm.containerfile)
2. Run the following command to create a custom Triton Inference Server w/ all necessary tools to generate TensorRT-LLM
   plan and engine files. In this example weâll use the tag `24.08` to match the date portion of `24.08-trtllm-python-py3`
   from the base image.
...
3. Upload the Container Image to a Cluster Visible Repository.
   ...
   In this example, weâll use the fictional `nvcr.io/example` repository for demonstration purposes.
   You will need to determine which repositories you have write access to that your cluster can also access.

   1. First, re-tag the container image with the repositoryâs name like below.
...
#### Kubernetes Pull Secrets[#](#kubernetes-pull-secrets "Link to this heading")
...
Weâll be using the `nvcr.io` container image repository example above for demonstration purposes.
...
## Triton Deployment[#](#triton-deployment "Link to this heading")

### Deploying Single GPU Models[#](#deploying-single-gpu-models "Link to this heading")
...
1. Create a custom values file with required values:
   ...
   * Image pull secrets (if necessary).
   ...
   The provided sample Helm [chart](https://github.com/triton-inference-server/tutorials/blob/main/Deployment/Kubernetes/TensorRT-LLM_Autoscaling_and_Load_Balancing/chart) include several example values files such as
   [llama-3-8b\_values.yaml](https://github.com/triton-inference-server/tutorials/blob/main/Deployment/Kubernetes/TensorRT-LLM_Autoscaling_and_Load_Balancing/chart/llama-3-8b-instruct_values.yaml).
...
3. Verify the Chart Installation.
   ...
   You should output similar to below (assuming the installation name of âllama-3â):
   ...
   Triton Server.
   Without inference queries, there are no metrics generated and thus the HPA controller reports the metricâs current values
   as `<unknown>`.
...
### Deploying Models Too Large for a Single GPU[#](#deploying-models-too-large-for-a-single-gpu "Link to this heading")
...
The provided sample Helm [chart](https://github.com/triton-inference-server/tutorials/blob/main/Deployment/Kubernetes/TensorRT-LLM_Autoscaling_and_Load_Balancing/chart) provides a mechanism for taking advantage of this capability.
...
### Utilizing Multiple GPU SKUs[#](#utilizing-multiple-gpu-skus "Link to this heading")
...
In the example below, weâll assume the service has already been created and its selector is set to `model=llama-3-8b`.
...
The result will be two deployments in your cluster, both of which are part of your serviceâs load-balancing pool.
...
### Monitoring Triton in Kubernetes[#](#monitoring-triton-in-kubernetes "Link to this heading")
...
4. The first thing weâll want to do is to create a new custom dashboard.
   To do this, click on the `+` icon in the upper-right of the user interface and select `New dashboard` from the dropdown menu.

   ![Visualization of the "new dashboard" interface](../../../../_images/grafana_new-dashboard.png)
5. Grafana will prompt you as to how you want to create a new dashboard.
   Select the `Import dashboard` option.

   ![Visualization of the "new dashboard" interface](../../../../_images/grafana_import-dashboard.png)
6. Either copy the content from the provided [grafana\_inference-metrics\_dashboard.json](https://github.com/triton-inference-server/tutorials/blob/main/Deployment/Kubernetes/TensorRT-LLM_Autoscaling_and_Load_Balancing/grafana_inference-metrics_dashboard.json) file and
...
7. Once youâve created the new dashboard, you should see something that looks like the image below.

   ![Example Grafana dashboard created by following the above instructions.](../../../../_images/grafana-dashboard.png)

Once the dashboard has been setup, you will be able to visualize the current state of your cluster.
These visualizations can provide insight into why weâve chosen to use the queue:compute ratio instead of GPU utilization as
the metric used to control the behavior of the horizontal pod autoscaler.

| GPU Utilization | Queue-to-Compute Ratio |
| --- | --- |
| Example GPU utilization graph | Example queue:compute ratio graph |
...
## Developing this Guide[#](#developing-this-guide "Link to this heading")
...
> *This document was developed using a Kubernetes cluster provided by Amazon EKS.*
> *Clusters provisioned on-premises or provided by other cloud service providers such as Azure AKS or GCloud GKE might require*
> *modifications to this guide.*

### Metrics Configuration is as Much an Art as a Science[#](#metrics-configuration-is-as-much-an-art-as-a-science "Link to this heading")
...
Much of the effort was spent on discovering the intricacies of Kubernetesâ HPA controller and how it consumed metrics.
...
well as in the Prometheus Rule we created in the above guide until everything âjust workedâ.

### Why This Set of Software Components?[#](#why-this-set-of-software-components "Link to this heading")
...
#### NVIDIA Device Plugin for Kubernetes[#](#id1 "Link to this heading")
...
#### NVIDIA GPU Discovery Service for Kubernetes[#](#nvidia-gpu-discovery-service-for-kubernetes "Link to this heading")
...
#### Kubernetes Node Discovery Service[#](#kubernetes-node-discovery-service "Link to this heading")

This is a requirement for the [NVIDIA GPU Discovery Service for Kubernetes](#nvidia-gpu-discovery-service-for-kubernetes).

#### NVIDIA DCGM Exporter[#](#nvidia-dcgm-exporter "Link to this heading")
...
only values that pertain to the individual process, and serving them via Tritonâs open-metrics server is as wasteful as the
...
##### Why is the DCGM Exporter Values File Custom?[#](#why-is-the-dcgm-exporter-values-file-custom "Link to this heading")
...
any container is unconstrained and quickly waste an entire engineering teamâs time as they attempt to triage, debug, and
resolve them.
...
#### Prometheus Stack for Kubernetes[#](#prometheus-stack-for-kubernetes "Link to this heading")
...
Additionally, the automatic inclusion of a Grafana based user interface made visualization of the clusterâs current health
easier to set up.
...
#### Prometheus Adapter for Kubernetes[#](#prometheus-adapter-for-kubernetes "Link to this heading")
...
##### Why the Custom Values File for Prometheus Adapter?[#](#why-the-custom-values-file-for-prometheus-adapter "Link to this heading")
...
### Why Does the Chart Run a Python Script Instead of Triton Server Directly?[#](#why-does-the-chart-run-a-python-script-instead-of-triton-server-directly "Link to this heading")
...
#### Why is the Python Written Like That?[#](#why-is-the-python-written-like-that "Link to this heading")

Because Iâm not a Python developer, but I am learning!
My background is in C/C++ with plenty of experience with shell scripting languages.

### What is the `client/` Folder For?[#](#what-is-the-client-folder-for "Link to this heading")
...
> Just like when building the `containers/triton_trt-llm.containerfile`, the image will need to hosted somewhere the clusterâs machines
> are able to download it from.

### Why Doesnât this Guide Include Load Balancer Instructions?[#](#why-doesn-t-this-guide-include-load-balancer-instructions "Link to this heading")
...
best instance to send new work to, showed modest-at-best improvements over the âround robinâ system provided by the
Kubernetesâ networking layer via [kube-proxy](https://kubernetes.io/docs/reference/command-line-tools-reference/kube-proxy/).
