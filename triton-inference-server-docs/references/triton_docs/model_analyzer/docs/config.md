...
# Configuring Model Analyzer[#](#configuring-model-analyzer "Link to this heading")
...
* `<range>`: An object containing `start` and `stop` key with an optional `step`
  ...
  + Types that support `<range>` can be described by a list or by using the example
    structure below, which declares the value of *batch\_sizes* to be an array `[2, 4, 6]`
...
# Config options for profile[#](#config-options-for-profile "Link to this heading")

## CLI and YAML Config Options[#](#cli-and-yaml-config-options "Link to this heading")
...
[ triton_docker_image: <string> | default: nvcr.io/nvidia/tritonserver:25.12-py3 ]
...
## YAML Only Options[#](#yaml-only-options "Link to this heading")
...
# Config Options for report[#](#config-options-for-report "Link to this heading")

## CLI and YAML options[#](#cli-and-yaml-options "Link to this heading")
...
## YAML only options[#](#id1 "Link to this heading")
...
# Field Descriptions[#](#field-descriptions "Link to this heading")
...
## `<constraint>`[#](#constraint "Link to this heading")
...
## `<objective>`[#](#objective "Link to this heading")
...
### Weighted Objectives[#](#weighted-objectives "Link to this heading")
...
to be âbetterâ than `measurement_B`. Model Analyzer will automatically account
...
## Test Configuration `<parameter>`[#](#test-configuration-parameter "Link to this heading")
...
## `<weighting>`[#](#weighting "Link to this heading")

This field is used to bias a modelâs objective when performing a multi-model search.

See [Multi-Model Search - Model Weighting](config_search.md#model-weighting) for details and an example YAML configuration.

## `<model-config-parameters>`[#](#model-config-parameters "Link to this heading")
...
## `<cpu_only>`[#](#cpu-only "Link to this heading")
...
## `<perf-analyzer-flags>`[#](#perf-analyzer-flags "Link to this heading")
...
### Global options to apply to all instances of Perf Analyzer[#](#global-options-to-apply-to-all-instances-of-perf-analyzer "Link to this heading")

---

The `perf_analyzer_flags` section can be specified globally to affect
perf\_analyzer instances across all models in the following way:
...
### Model-specific options for Perf Analyzer[#](#model-specific-options-for-perf-analyzer "Link to this heading")
...
### Shape, Input-Data, and Streaming[#](#shape-input-data-and-streaming "Link to this heading")

---

The `input-data`, `shape`, and `streaming` perf\_analyzer options are additive and can take either
...
### Variable-sized dimensions[#](#variable-sized-dimensions "Link to this heading")
...
[Perf Analyzer documentation](../../perf_analyzer/docs/input_data.md).

### SSL Support:[#](#ssl-support "Link to this heading")
...
More information about this can be found in the
[Perf Analyzer documentation](../../perf_analyzer/docs/measurements_metrics.md#ssl-tls-support).

#### **Important Notes**:[#](#important-notes "Link to this heading")

* Only a subset of flags can be specified on the command line. Use `model-analyzer profile --help` to see the list of flags that can be specified on the command line. If a flag isnât listed there, it can be specified via the YAML config file.
...
## `<triton-server-flags>`[#](#triton-server-flags "Link to this heading")
...
More information about this can be found in the
[Triton Server documentation](../../customization_guide/inference_protocols.md#ssl-tls).

### **Important Notes**:[#](#id2 "Link to this heading")

* The Model Analyzer also provides certain arguments to the `tritonserver`
  instances it launches. These ***cannot*** be overridden by providing those
  arguments in this section. An example of this is `http-port`, which is an
  argument to Model Analyzer itself.

## `<triton-server-environment>`[#](#triton-server-environment "Link to this heading")
...
required by a model, Triton requires the LD\_PRELOAD and LD\_LIBRARY\_PATH
environment variables to be set. See [this link](../../user_guide/custom_operations.md)
...
## `<profile-model>`[#](#profile-model "Link to this heading")
...
## `<report-model-config>`[#](#report-model-config "Link to this heading")
...
## Config Defaults[#](#config-defaults "Link to this heading")

Up to date config defaults can be found in
[config\_defaults.py](https://github.com/triton-inference-server/model_analyzer/blob/main/model_analyzer/config/input/config_defaults.py)
