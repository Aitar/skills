...
# Triton Example Backends[#](#triton-example-backends "Link to this heading")
...
* The
  ...
  Management](../../user_guide/architecture.md#implicit-state-management)
...
### *Minimal* Triton Backend[#](#minimal-triton-backend "Link to this heading")

The source code for the *minimal* backend is contained in
[minimal.cc](https://github.com/triton-inference-server/backend/blob/main/examples/backends/minimal/src/minimal.cc). The source code
...
#### Building the *Minimal* Backend[#](#building-the-minimal-backend "Link to this heading")

[backends/minimal/CMakeLists.txt](https://github.com/triton-inference-server/backend/blob/main/examples/backends/minimal/CMakeLists.txt)
...
are building the r21.10 identity\_backend branch then you need to use
...
#### Running Triton with the *Minimal* Backend[#](#running-triton-with-the-minimal-backend "Link to this heading")
...
[model\_repos/minimal\_models](https://github.com/triton-inference-server/backend/blob/main/examples/model_repos/minimal_models). Assuming you
...
the *batching* and *nonbatching* models from the minimal\_models
...
the [batch
delay](../../user_guide/model_configuration.md#delayed-batching)
...
#### Testing the *Minimal* Backend[#](#testing-the-minimal-backend "Link to this heading")
...
[minimal\_client](https://github.com/triton-inference-server/backend/blob/main/examples/clients/minimal_client) Python script demonstrates
...
The minimal\_client first sends a single request to nonbatching
...
The minimal\_client next sends 2 requests at the same time to the
...
### *Recommended* Triton Backend[#](#recommended-triton-backend "Link to this heading")

The source code for the *recommended* backend is contained in
[recommended.cc](https://github.com/triton-inference-server/backend/blob/main/examples/backends/recommended/src/recommended.cc). The source
...
* Uses the Triton backend metric APIs to record statistics about
  ...
  and
  [statistics](../../protocol/extension_statistics.md)
  APIs.
* Additional error checking to ensure that the backendâs version is
  compatible with Triton and that each modelâs configuration is
...
#### Building the *Recommended* Backend[#](#building-the-recommended-backend "Link to this heading")

[backends/recommended/CMakeLists.txt](https://github.com/triton-inference-server/backend/blob/main/examples/backends/recommended/CMakeLists.txt)
shows the recommended build and install script for a Triton
backend. Building and installing is the same as described in [Building
the *Minimal* Backend](#building-the-minimal-backend).

#### Running Triton with the *Recommended* Backend[#](#running-triton-with-the-recommended-backend "Link to this heading")
...
[model\_repos/recommended\_models](https://github.com/triton-inference-server/backend/blob/main/examples/model_repos/recommended_models). Assuming
...
The console output will show similar to the following indicating that
the *batching* model from the recommended\_models repository have
loaded correctly.
...
#### Testing the *Recommended* Backend[#](#testing-the-recommended-backend "Link to this heading")

The [clients](https://github.com/triton-inference-server/backend/blob/main/examples/clients) directory holds example clients. The
[recommended\_client](https://github.com/triton-inference-server/backend/blob/main/examples/clients/recommended_client) Python script
...
The recommended\_client next sends 2 requests at the same time to the
...
To see the metrics collected for these two inference requests, use the following command to access Tritonâs metrics endpoint.
...
[metrics
documentation](../../user_guide/metrics.md)
...
endpoint](../../protocol/extension_statistics.md).
...
### Custom Batching[#](#custom-batching "Link to this heading")
...
Two example batching libraries are located in the [batching\_strategies directory](https://github.com/triton-inference-server/backend/blob/main/examples/batching_strategies).

For this tutorial, you can use the [volume\_batching](https://github.com/triton-inference-server/backend/blob/main/examples/batching_strategies/volume_batching) example
...
model, and backend directories, in that order. For ease, weâll pass it via the model configuration.
...
[max queue delay](../../user_guide/model_configuration.md#delayed-batching)
...
#### Automatically Model Configuration Generation[#](#automatically-model-configuration-generation "Link to this heading")

[Automatic model configuration
generation](../../user_guide/model_configuration.md#auto-generated-model-configuration)
is enabled by the backend implementing the appropriate logic (for
example, in a function called AutoCompleteConfig) during
TRITONBACKEND\_ModelInitialize. For the *recommended* backend you would
...
configuration using TRITONBACKEND\_ModelSetConfig. Examples can be
...
#### Add Key-Value Parameters to a Response[#](#add-key-value-parameters-to-a-response "Link to this heading")
...
#### Access Model Artifacts in the Model Repository[#](#access-model-artifacts-in-the-model-repository "Link to this heading")

A backend can access any of the files in a modelâs area of the model
registry. These files are typically needed during
TRITONBACKEND\_ModelInitialize but can be accessed at other times as
well. The TRITONBACKEND\_ModelRepository API gives the location of the
modelâs repository. For example, the following code can be run during
TRITONBACKEND\_ModelInitialize to write the location to the log.
