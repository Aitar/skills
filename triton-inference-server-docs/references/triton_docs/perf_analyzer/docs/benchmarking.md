...
# Benchmarking Triton via HTTP or gRPC endpoint[#](#benchmarking-triton-via-http-or-grpc-endpoint "Link to this heading")

This is the default mode for Perf Analyzer.

# Benchmarking OpenAI[#](#benchmarking-openai "Link to this heading")
...
## Benchmarking OpenAI Multi-Turn Chat[#](#benchmarking-openai-multi-turn-chat "Link to this heading")
...
# Benchmarking Triton directly via C API[#](#benchmarking-triton-directly-via-c-api "Link to this heading")
...
## Prerequisite[#](#prerequisite "Link to this heading")
...
[`--triton-server-directory`](cli.md#triton-server-directory-path) option and
the model repository path using the
[`--model-repository`](cli.md#model-repository-path) option.
...
## Non-supported functionalities[#](#non-supported-functionalities "Link to this heading")
...
2. For additional known non-working cases, please refer to
   [qa/L0\_perf\_analyzer\_capi/test.sh](https://github.com/triton-inference-server/server/blob/main/qa/L0_perf_analyzer_capi/test.sh#L239-L277)

# Benchmarking gRPC service via Dynamic gRPC[#](#benchmarking-grpc-service-via-dynamic-grpc "Link to this heading")

> **Note**
>
> Dynamic gRPC service kind does not support asynchronous gRPC APIs at the moment.

## Setting up mock gRPC service[#](#setting-up-mock-grpc-service "Link to this heading")

Before getting into the details of how to run Perf Analyzer via Dynamic gRPC service kind,
letâs setup a mock gRPC service that we can use to test the Dynamic gRPC service kind.
...
## Message Framing Protocol[#](#message-framing-protocol "Link to this heading")
...
## Input JSON file[#](#input-json-file "Link to this heading")
...
# Benchmarking TensorFlow Serving[#](#benchmarking-tensorflow-serving "Link to this heading")
...
1. `Concurrent Request Execution`:
   ...
   [MAX\_SESSION\_SHARE\_COUNT](https://github.com/triton-inference-server/tensorflow_backend#parameters)
...
# Benchmarking TorchServe[#](#benchmarking-torchserve "Link to this heading")
...
# Advantages of using Perf Analyzer over third-party benchmark suites[#](#advantages-of-using-perf-analyzer-over-third-party-benchmark-suites "Link to this heading")
...
1. Using
   [binary tensor data extension](../../protocol/extension_binary_data.md#binary-tensor-data-extension)
   with HTTP requests.
