...
### Install GenAI-Perf (Ubuntu 24.04, Python 3.10+)[#](#install-genai-perf-ubuntu-24-04-python-3-10 "Link to this heading")
...
â¡ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ©
â          Time to first token (ms) â  16.26 â  12.39 â  17.25 â  17.09 â  16.68 â  16.56 â
â          Inter token latency (ms) â   1.85 â   1.55 â   2.04 â   2.02 â   1.97 â   1.92 â
â              Request latency (ms) â 499.20 â 451.01 â 554.61 â 548.69 â 526.13 â 514.19 â
â            Output sequence length â 261.90 â 256.00 â 298.00 â 296.60 â 270.00 â 265.00 â
â             Input sequence length â 550.06 â 550.00 â 553.00 â 551.60 â 550.00 â 550.00 â
â Output token throughput (per sec) â 520.87 â    N/A â    N/A â    N/A â    N/A â    N/A â
â      Request throughput (per sec) â   1.99 â    N/A â    N/A â    N/A â    N/A â    N/A â
âââââââââââââââââââââââââââââââââââââ´âââââââââ´âââââââââ´âââââââââ´âââââââââ´âââââââââ´âââââââââ
```

See [Tutorial](docs/tutorial.md) for additional examples.

## Configuration File[#](#configuration-file "Link to this heading")
...
### Creating a Template Config File[#](#creating-a-template-config-file "Link to this heading")
...
### Overriding Config Options[#](#overriding-config-options "Link to this heading")
...
## Analyze[#](#analyze "Link to this heading")
...
## Process Export Files[#](#process-export-files "Link to this heading")
...
* `--prefix-prompt-length <int>`: The number of tokens to include in each
  prefix prompt. This value is only used if ânum-prefix-prompts is positive.
...
### Use moon\_cake file as input payload[#](#use-moon-cake-file-as-input-payload "Link to this heading")
...
The payload file is in [moon\_cake format](https://github.com/kvcache-ai/Mooncake) which contains a `timestamp` field and you can optionally add `input_length`, `output_length`, `text_input`, `session_id`, `hash_ids` and `priority`. For further examples using these fields, you can refer to our [multi-turn session benchmark tutorial](docs/multi_turn.md#approach-2-benchmark-with-a-custom-dataset).
...
`hash_ids` are a list of hash\_id, each of which maps to a unique synthetic prompt sequence with `block_size` (current block size is 512) of tokens after tokenizer encoding, defined in [create\_synthetic\_prompt](https://github.com/triton-inference-server/perf_analyzer/blob/main/genai-perf/genai_perf/inputs/retrievers/synthetic_prompt_generator.py#L37). Since the same hash\_id maps to the same input block, it is effective to test features such as kv cache and speculative decoding.

### How to generate a payload file[#](#how-to-generate-a-payload-file "Link to this heading")

#### 1. Synthetic data from sampling[#](#synthetic-data-from-sampling "Link to this heading")

[Nvidia Dynamo](https://github.com/ai-dynamo/dynamo) provides [a script](https://github.com/ai-dynamo/dynamo/blob/main/docs/guides/planner_benchmark/sin_synth.py) (with [README](https://github.com/ai-dynamo/dynamo/blob/main/docs/guides/planner_benchmark/benchmark_planner.md)) to generate synthetic moon\_cake style payload:
...
#### 2. Record real traffic and replay[#](#record-real-traffic-and-replay "Link to this heading")

We recommend users to build a traffic footprint collector over their inference service to generate the moon\_cake format payload file based on real service traffic. Although popular inference servers do not support this feature, users can add this feature to their inference service.
...
### GPU Telemetry[#](#gpu-telemetry "Link to this heading")
...
### Power Metrics[#](#power-metrics "Link to this heading")

* `gpu_power_usage`
* `gpu_power_limit`
* `energy_consumption`

### Memory Metrics[#](#memory-metrics "Link to this heading")

* `total_gpu_memory`
* `gpu_memory_used`
* `gpu_memory_free`

### Temperature Metrics[#](#temperature-metrics "Link to this heading")

* `gpu_temperature`
* `gpu_memory_temperature`

### Clock Metrics[#](#clock-metrics "Link to this heading")

* `gpu_clock_sm`
* `gpu_clock_memory`

### Utilization Metrics[#](#utilization-metrics "Link to this heading")
...
### ECC Metrics[#](#ecc-metrics "Link to this heading")
...
### Errors and Violations Metrics[#](#errors-and-violations-metrics "Link to this heading")

* `xid_last_error`
* `power_throttle_duration`
* `thermal_throttle_duration`

### Retired Pages Metrics[#](#retired-pages-metrics "Link to this heading")

* `retired_pages_sbe`
* `retired_pages_dbe`

### NVLink Error Metrics[#](#nvlink-error-metrics "Link to this heading")

* `total_nvlink_crc_flit_errors`
* `total_nvlink_crc_data_errors`

### PCIE Metrics[#](#pcie-metrics "Link to this heading")
...
Use the provided [`custom-metrics.csv`](https://github.com/triton-inference-server/perf_analyzer/blob/main/genai-perf/docs/assets/custom_gpu_metrics.csv) to ensure all required metrics are included in the output.
...
#### `-f`[#](#f "Link to this heading")

#### `--file`[#](#file "Link to this heading")

The path to the config file - REQUIRED.

#### `--override-config`[#](#override-config "Link to this heading")

An option that allows the user to override values specified in the config file.

### Template Options[#](#template-options "Link to this heading")

#### `-f`[#](#id1 "Link to this heading")

#### `--file`[#](#id2 "Link to this heading")
...
#### `-m <list>`[#](#m-list "Link to this heading")

#### `--model <list>`[#](#model-list "Link to this heading")
...
#### `--model-selection-strategy {round_robin, random}`[#](#model-selection-strategy-round-robin-random "Link to this heading")
...
#### `--backend {tensorrtllm,vllm}`[#](#backend-tensorrtllm-vllm "Link to this heading")

When benchmarking Triton, this is the backend of the model.
(default: tensorrtllm)

#### `--endpoint <str>`[#](#endpoint-str "Link to this heading")

Set a custom endpoint that differs from the OpenAI defaults. (default: `None`)

#### `--endpoint-type <str>`[#](#endpoint-type-str "Link to this heading")

The endpoint-type to send requests to on the server. (default: `kserve`)

#### `--server-metrics-urls <list>`[#](#server-metrics-urls-list "Link to this heading")

The list of server metrics URLs. These are used for Telemetry metric
reporting when benchmarking. Example usage: âserver-metrics-urls
...
#### `--url <url>`[#](#url-url "Link to this heading")

URL of the endpoint to target for benchmarking. (default: `None`)

#### `--grpc-method <str>`[#](#grpc-method-str "Link to this heading")

A fully-qualified gRPC method name in â./â format.
...
#### `--audio-length-mean <int>`[#](#audio-length-mean-int "Link to this heading")

The mean length of audio data in seconds. (default: `0`)

#### `--audio-length-stddev <int>`[#](#audio-length-stddev-int "Link to this heading")

The standard deviation of the length of audio data in seconds.
(default: `0`)

#### `--audio-format <str>`[#](#audio-format-str "Link to this heading")

The format of the audio data. Currently we support wav and mp3 format.
(default: `wav`)

#### `--audio-depths <int>`[#](#audio-depths-int "Link to this heading")

A list of audio bit depths to randomly select from in bits.
(default: `[16]`)

#### `--audio-sample-rates <int>`[#](#audio-sample-rates-int "Link to this heading")

A list of audio sample rates to randomly select from in kHz.
(default: `[16]`)

#### `--audio-num-channels <int>`[#](#audio-num-channels-int "Link to this heading")
...
#### `-b <int>`[#](#b-int "Link to this heading")

#### `--batch-size <int>`[#](#batch-size-int "Link to this heading")

#### `âbatch-size-text [#](#batch-size-text "Link to this heading")

The text batch size of the requests GenAI-Perf should send.
(default: `1`)

#### `âbatch-size-text [#](#id3 "Link to this heading")

The text batch size of the requests GenAI-Perf should send.
(default: `1`)

#### `--batch-size-audio <int>`[#](#batch-size-audio-int "Link to this heading")
...
#### `--batch-size-text <int>`[#](#batch-size-text-int "Link to this heading")
...
#### `--batch-size-image <int>`[#](#batch-size-image-int "Link to this heading")
...
#### `--extra-inputs <str>`[#](#extra-inputs-str "Link to this heading")

Provide additional inputs to include with every request. You can repeat this
flag for multiple inputs. Inputs should be in an input\_name:value format.
Alternatively, a string representing a json formatted dict can be provided.
(default: `None`)

#### `--header <str>`[#](#header-str "Link to this heading")

#### `--H <str>`[#](#h-str "Link to this heading")

Add a custom header to the requests. Headers must be specified as
âHeader:Valueâ. You can repeat this flag for multiple headers.
(default: `None`)

#### `--input-file <path>`[#](#input-file-path "Link to this heading")
...
#### `--num-dataset-entries <int>`[#](#num-dataset-entries-int "Link to this heading")

The number of unique payloads to sample from. These will be reused until
benchmarking is complete. (default: `100`)

#### `--num-prefix-prompts <int>`[#](#num-prefix-prompts-int "Link to this heading")
...
#### `--output-tokens-mean <int>`[#](#output-tokens-mean-int "Link to this heading")

#### `--osl`[#](#osl "Link to this heading")

The mean number of tokens in each output. Ensure the `--tokenizer` value is set
correctly. (default: `-1`)

#### `--output-tokens-mean-deterministic`[#](#output-tokens-mean-deterministic "Link to this heading")
...
#### `--output-tokens-stddev <int>`[#](#output-tokens-stddev-int "Link to this heading")
...
#### `--random-seed <int>`[#](#random-seed-int "Link to this heading")

The seed used to generate random values. If not provided, a random seed will be
used.

#### `--request-count <int>`[#](#request-count-int "Link to this heading")

#### `--num-requests <int>`[#](#num-requests-int "Link to this heading")

The number of requests to use for measurement.
(default: `10`)

#### `--synthetic-input-tokens-mean <int>`[#](#synthetic-input-tokens-mean-int "Link to this heading")

#### `--isl`[#](#isl "Link to this heading")

The mean of number of tokens in the generated prompts when using synthetic
data. (default: `550`)

#### `--synthetic-input-tokens-stddev <int>`[#](#synthetic-input-tokens-stddev-int "Link to this heading")

The standard deviation of number of tokens in the generated prompts when
using synthetic data. (default: `0`)

#### `--prefix-prompt-length <int>`[#](#prefix-prompt-length-int "Link to this heading")

The number of tokens in each prefix prompt. This value is only used if
ânum-prefix-prompts is positive. Note that due to the prefix and user prompts
being concatenated, the number of tokens in the final prompt may be off by one.
(default: `100`)

#### `--image-width-mean <int>`[#](#image-width-mean-int "Link to this heading")

The mean width of images in pixels when generating synthetic image data.
(default: `0`)

#### `--image-width-stddev <int>`[#](#image-width-stddev-int "Link to this heading")

The standard deviation of width of images in pixels when generating synthetic image data.
(default: `0`)

#### `--image-height-mean <int>`[#](#image-height-mean-int "Link to this heading")

The mean height of images in pixels when generating synthetic image data.
(default: `0`)

#### `--image-height-stddev <int>`[#](#image-height-stddev-int "Link to this heading")

The standard deviation of height of images in pixels when generating synthetic image data.
(default: `0`)

#### `--image-format <str>`[#](#image-format-str "Link to this heading")

The compression format of the images. If format is not selected,
format of generated image is selected at random.

#### `--warmup-request-count <int>`[#](#warmup-request-count-int "Link to this heading")

#### `--num-warmup-requests <int>`[#](#num-warmup-requests-int "Link to this heading")
...
#### `--concurrency <int>`[#](#concurrency-int "Link to this heading")

The concurrency value to benchmark. (default: `None`)

#### `--measurement-interval <int>`[#](#measurement-interval-int "Link to this heading")

#### `-p <int>`[#](#p-int "Link to this heading")
...
for 3\*(measurement\_interval) milliseconds.

#### `--request-rate <float>`[#](#request-rate-float "Link to this heading")
...
#### `-s <float>`[#](#s-float "Link to this heading")

#### `--stability-percentage <float>`[#](#stability-percentage-float "Link to this heading")
...
The path where the perf\_analyzer profile export will be generated. By default,
...
### Session Options[#](#session-options "Link to this heading")

#### `--num-sessions`[#](#num-sessions "Link to this heading")

The number of sessions to simulate. This is used when generating synthetic
session data. (default: `0`)

#### `--session-concurrency <int>`[#](#session-concurrency-int "Link to this heading")

The number of concurrent sessions to benchmark. This must be specified
when benchmarking sessions. (default: `0`)

#### `--session-delay-ratio <float>`[#](#session-delay-ratio-float "Link to this heading")
...
#### `--session-turn-delay-mean`[#](#session-turn-delay-mean "Link to this heading")

The mean delay (in milliseconds) between turns in a session.
(default: `0`)

#### `--session-turn-delay-stddev`[#](#session-turn-delay-stddev "Link to this heading")

The standard deviation of the delay (in milliseconds) between turns in a session.
(default: `0`)

#### `--session-turns-mean`[#](#session-turns-mean "Link to this heading")

The mean number of turns per session.
(default: `1`)

#### `--session-turns-stddev`[#](#session-turns-stddev "Link to this heading")

The standard deviation of the number of turns per session.
(default: `0`)

### Tokenizer Options[#](#tokenizer-options "Link to this heading")

#### `--tokenizer <str>`[#](#tokenizer-str "Link to this heading")
...
(default: â<model\_value>â)

#### `--tokenizer-revision <str>`[#](#tokenizer-revision-str "Link to this heading")

The specific tokenizer model version to use. It can be a branch
name, tag name, or commit ID. (default: `main`)

#### `--tokenizer-trust-remote-code`[#](#tokenizer-trust-remote-code "Link to this heading")
...
#### `--verbose`[#](#verbose "Link to this heading")
...
#### `-g <list>`[#](#g-list "Link to this heading")

#### `--goodput <list>`[#](#goodput-list "Link to this heading")

An option to provide constraints in order to compute goodput. Specify goodput
constraints as âkey:valueâ pairs, where the key is a valid metric name, and the
value is a number representing either milliseconds or a throughput value per
second. For example, ârequest\_latency:300â or
âoutput\_token\_throughput\_per\_user:600â. Multiple key:value pairs can be
