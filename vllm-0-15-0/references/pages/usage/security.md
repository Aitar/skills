...
Source: https://docs.vllm.ai/en/v0.15.0/usage/security/
...
Bypass authentication by using non-`/v1` endpoints like `/invocations`, `/inference/v1/generate`, `/pooling`, `/classify`, `/score`, or `/rerank` to run arbitrary inference without credentials

Cause denial of service by calling `/pause` or `/scale_elastic_ep` without a token

Access operational controls to manipulate server state (e.g., pausing generation)

If `--enable-tokenizer-info-endpoint` is set: Access sensitive tokenizer configuration including chat templates, which may reveal prompt engineering strategies or other implementation details

If `VLLM_SERVER_DEV_MODE=1` is set: Execute arbitrary RPC commands via `/collective_rpc`, reset caches, put the engine to sleep, and access detailed server configuration
