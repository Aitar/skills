...
Source: https://docs.vllm.ai/en/v0.15.0/features/multimodal_inputs/
...
Detects if the model requires mono audio via the feature extractor

Converts multi-channel audio to mono using channel averaging

Handles both `(channels, time)` format (torchaudio) and `(time, channels)` format (soundfile)
...
The system reads frames sequentially

If a target frame fails to grab, it's marked as "failed"

The next successfully grabbed frame (before reaching the next target) is used to recover the failed frame

This approach handles both mid-video corruption and end-of-video truncation
