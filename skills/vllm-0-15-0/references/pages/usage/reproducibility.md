...
Source: https://docs.vllm.ai/en/v0.15.0/usage/reproducibility/
...
Setting `VLLM_ENABLE_V1_MULTIPROCESSING=0` will change the random state of user code (i.e. the code that constructs LLM class).
...
It is impossible to un-specify a seed for V1 because different workers need to sample the same outputs for workflows such as speculative decoding. For more information, see: Pull Request #17929

Note

The random state in user code (i.e. the code that constructs LLM class) is updated by vLLM only if the workers are run in the same process as user code, i.e.: `VLLM_ENABLE_V1_MULTIPROCESSING=0`.
