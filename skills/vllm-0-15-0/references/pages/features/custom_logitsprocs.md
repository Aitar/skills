...
Source: https://docs.vllm.ai/en/v0.15.0/features/custom_logitsprocs/
...
Example of Wrapping a Request-Level Logits Processor
...
See examples below:
Passing custom logits processor FQCN to `LLM` in Python
...
Passing custom logits processor FQCN to `AsyncLLM` in Python
...
Passing custom logits processor FQCN to vLLM server via CLI
...
Exposing a custom logits processor as a Python entrypoint
...
Passing custom logits processor class object to `LLM` or `AsyncLLM` in Python
...
vLLM REST API: configure custom logits processor for a request
...
OpenAI SDK: configure custom logits processor for a request
...
Offline: configure custom logits processor for an `LLM` request
...
Offline: configure custom logits processor for an `AsyncLLM` request
