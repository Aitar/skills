...
# Launch Modes[#](#launch-modes "Link to this heading")

Triton Model Analyzerâs `profile` subcommand supports four different launch
...
[C\_API](../../customization_guide/inference_protocols.md#in-process-triton-server-api)
by the perf\_analyzer instances launched by Model Analyzer. Inference requests are
...
Server. You may provide the URLs for the Triton instanceâs HTTP or GRPC endpoint
