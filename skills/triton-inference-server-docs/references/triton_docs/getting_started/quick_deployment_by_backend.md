...
# Quick Deployment Guide by backend[#](#quick-deployment-guide-by-backend "Link to this heading")
...
tutorials](../tutorials/README.md#quick-deploy) to begin your Triton
journey!

The Triton Inference Server is available as [buildable source
code](../customization_guide/build.md), but the easiest way to
install and run Triton is to use the pre-built Docker image available
from the [NVIDIA GPU Cloud (NGC)](https://ngc.nvidia.com).
...
The [model repository](../user_guide/model_repository.md) is the
...
All the models should show âREADYâ status to indicate that they loaded
...
On a system without GPUs, Triton should be run without using the âgpus
...
using the example densenet\_onnx model.
