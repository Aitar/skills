...
# GenAI-Perf Analyze Subcommand[#](#genai-perf-analyze-subcommand "Link to this heading")
...
## Analyze CLI[#](#analyze-cli "Link to this heading")
...
### `--sweep-type` - The type of stimulus you wish the sweep over[#](#sweep-type-the-type-of-stimulus-you-wish-the-sweep-over "Link to this heading")
...
### `--sweep-range` - The range over which the stimulus will be swept[#](#sweep-range-the-range-over-which-the-stimulus-will-be-swept "Link to this heading")
...
### `--sweep-list` - A comma-separated list of values that stimulus will be swept over[#](#sweep-list-a-comma-separated-list-of-values-that-stimulus-will-be-swept-over "Link to this heading")
...
This will sweep over request rate at values of 100,150,â¦450,500
...
## Artifact Directories and Summary CSV Report[#](#artifact-directories-and-summary-csv-report "Link to this heading")
...
## Summary Report CSV[#](#summary-report-csv "Link to this heading")

In the CWD a summary report CSV (`analyze_export_genai_perf.csv`) is created. In the first table, each row is a different scenario profiled while the columns show p99 perf metrics. In the second table, the rows are again scenarioâs profiled, while the columns are p99 GPU telemetry metrics.

### Example Summary Report CSV[#](#example-summary-report-csv "Link to this heading")
...
## Checkpointing[#](#checkpointing "Link to this heading")

A new feature of `analyze` is the ability to save and restore results from scenarios that have previously run. This is done via reading a `checkpoint.json` (which is located at the CWD). This checkpoint file contains all the information necessary for GenAI-Perf to restore itâs state from a previous run of `analyze` and skip over any scenarios that have previously been profiled.
...
This command will profile three scenarios: num\_dataset\_entries with values 100, 150, & 200.
...
## Reading the Checkpoint and Using the Results API[#](#reading-the-checkpoint-and-using-the-results-api "Link to this heading")
...
### Results, RunConfigs, and Records Classes[#](#results-runconfigs-and-records-classes "Link to this heading")
...
### Record Class[#](#record-class "Link to this heading")
...
## Results API[#](#results-api "Link to this heading")
...
### RunConfig API[#](#runconfig-api "Link to this heading")
...
### Example Python Code[#](#example-python-code "Link to this heading")
