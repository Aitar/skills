* [vLLM](../backend_guide/vllm.md)
* vLLM Backend

[![License](https://img.shields.io/badge/License-BSD3-lightgrey.svg)](https://opensource.org/licenses/BSD-3-Clause)

# vLLM Backend[#](#vllm-backend "Link to this heading")
...
This is a [Python-based backend](../backend/docs/python_based_backends.md#python-based-backends).
...
## Installing the vLLM Backend[#](#installing-the-vllm-backend "Link to this heading")

There are several ways to install and deploy the vLLM backend.

### Option 1. Use the Pre-Built Docker Container.[#](#option-1-use-the-pre-built-docker-container "Link to this heading")
...
that Tritonâs vLLM container has been introduced starting from 23.10 release.

```
docker pull nvcr.io/nvidia/tritonserver:<xx.yy>-vllm-python-py3
```

### Option 2. Build a Custom Container From Source[#](#option-2-build-a-custom-container-from-source "Link to this heading")

You can follow steps described in the
[Building With Docker](../customization_guide/build.md#building-with-docker)
guide and use the
...
### Option 3. Add the vLLM Backend to the Default Triton Container[#](#option-3-add-the-vllm-backend-to-the-default-triton-container "Link to this heading")
...
## Using the vLLM Backend[#](#using-the-vllm-backend "Link to this heading")
...
`model.json` represents a key-value dictionary that is fed to vLLMâs AsyncLLMEngine when initializing the model.
You can see supported arguments in vLLMâs
...
### Launching Triton Inference Server[#](#launching-triton-inference-server "Link to this heading")
...
Note that Tritonâs vLLM container was first published starting from
23.10 release.
...
### Sending Your First Inference[#](#sending-your-first-inference "Link to this heading")

After you
[start Triton](https://docs.nvidia.com/deeplearning/triton-inference-server/user-guide/docs/getting_started/quickstart.md)
with the
...
[generate endpoint](../protocol/extension_generate.md).
...
[client.py](https://github.com/triton-inference-server/vllm_backend/blob/main/samples/client.py) which uses Tritonâs
[asyncio gRPC client library](../client/README.md#id3)
to run inference on Triton.

### Running the Latest vLLM Version[#](#running-the-latest-vllm-version "Link to this heading")

You can check the vLLM version included in Triton Inference Server from
[Framework Containers Support Matrix](https://docs.nvidia.com/deeplearning/frameworks/support-matrix/index.md).
...
## Additional vLLM outputs[#](#additional-vllm-outputs "Link to this heading")
...
[here](https://docs.vllm.ai/en/latest/serving/metrics.md)). This can be
...
To enable vLLM engine colleting metrics, âdisable\_log\_statsâ option need to be either false
...
due to potential performance slowdowns. To enable vLLM modelâs metrics
...
## vLLM Engine Health Check (BETA)[#](#vllm-engine-health-check-beta "Link to this heading")
...
## Referencing the Tutorial[#](#referencing-the-tutorial "Link to this heading")
