...
# Deploy models using Triton[#](#deploy-models-using-triton "Link to this heading")

| Navigate to | [Part 2: Improving Resource Utilization](https://github.com/triton-inference-server/tutorials/blob/main/Conceptual_Guide/Part_2-improving_resource_utilization) | [Documentation: Model Repository](../../../user_guide/model_repository.md) | [Documentation: Model Configuration](../../../user_guide/model_configuration.md) |
| --- | --- | --- | --- |
...
## Before we begin[#](#before-we-begin "Link to this heading")
...
2. Detect which parts of the image contain text (Text Detection Model)
3. Crop image to regions with text
4. Find text probabilities (Text Recognition Model)
5. Convert probabilities to actual text

In `Part 1`, we start by deploying both models on Triton with the pre/post processing steps done on the client.

## Deploying multiple models[#](#deploying-multiple-models "Link to this heading")
...
![multiple models](../../../_images/multiple_models.PNG)

The first step in deploying models using the Triton Inference Server is building a repository that houses the models which will be served and the configuration schema. For the purposes of this demonstration, we will be making use of an [EAST](https://arxiv.org/pdf/1704.03155v2.pdf) model to detect text and a text recognition model. This workflow is largely an adaptation of [OpenCVâs Text Detection](https://docs.opencv.org/4.x/db/da4/samples_2dnn_2text_detection_8cpp-example.md) samples.

To begin, letâs clone the repository and navigate to this folder.

```
cd Conceptual_Guide/Part_1-model_deployment
```

Next, weâll be downloading the necessary models and making sure they are in a format that triton can deploy.

### Model 1: Text Detection[#](#model-1-text-detection "Link to this heading")

Download and unzip OpenCVâs EAST model.
...
### Model 2: Text Recognition[#](#model-2-text-recognition "Link to this heading")
...
### Setting up the model repository[#](#setting-up-the-model-repository "Link to this heading")

A [model repository](https://docs.nvidia.com/deeplearning/triton-inference-server/user-guide/docs/user_guide/model_repository.md) is Tritonâs way of reading your models and any associated metadata with each model (configurations, version files, etc.). These model repositories can live in a local or network attached filesystem, or in a cloud object store like AWS S3, Azure Blob Storage or Google Cloud Storage. For more details on model repository location, refer to [the documentation](https://docs.nvidia.com/deeplearning/triton-inference-server/user-guide/docs/user_guide/model_repository.md#model-repository-locations). Servers can use also multiple different model repositories. For simplicity, this explanation only uses a single repository stored in the [local filesystem](https://docs.nvidia.com/deeplearning/triton-inference-server/user-guide/docs/user_guide/model_repository.md#local-file-system), in the following format:
...
* `version`: versioning makes multiple versions of the same model available for use depending on the policy selected. [More Information about versioning.](https://docs.nvidia.com/deeplearning/triton-inference-server/user-guide/docs/user_guide/model_repository.md#model-versions)
...
Note that, for this example, weâve already created the `config.pbtxt` files and placed them in the necessary location. In the next section, weâll discuss the contents of these files.

### Model configuration[#](#model-configuration "Link to this heading")

With the models and the file structure ready, the next things we need to look at are the `config.pbtxt` model configuration files. Letâs first look at the model configuration for the `EAST text detection` model thatâs been provided for you at `/model_repository/text_detection/config.pbtxt`. This shows that `text_detection` is an ONNX model that has one `input` and two `output` tensors.
...
* `name`: ânameâ is an optional field, the value of which should match the name of the directory of the model.
* `backend`: This field indicates which backend is being used to run the model. Triton supports a wide variety of backends like TensorFlow, PyTorch, Python, ONNX and more. For a complete list of field selection refer to [these comments](../../../backend/README.md#backends).
* `max_batch_size`: As the name implies, this field defines the maximum batch size that the model can support.
* `input` and `output`: The input and output sections specify the name, shape, datatype, and more, while providing operations like [reshaping](../../../user_guide/model_configuration.md#reshape) and support for [ragged batches](../../../user_guide/ragged_batching.md#ragged-batching).

In [most cases](https://docs.nvidia.com/deeplearning/triton-inference-server/user-guide/docs/user_guide/model_configuration.md#auto-generated-model-configuration), itâs possible to leave out the `input` and `output` sections and let Triton extract that information from the model files directly. Here, weâve included them for clarity and because weâll need to know the names of our output tensors in the client application later on.
...
## Building a client application[#](#building-a-client-application "Link to this heading")
...
* First, we import our HTTP client from the `tritonclient` library, as well as a few other libraries weâll use for processing our images:
  ...
  ```
* Next, weâll define a few helper functions for taking care of the pre and post processing steps for our pipeline. The details are omitted here for brevity, but you can check the `client.py` file for more details
...
* Then, we create a client object, and initialize a connection with the Triton Inference Server.
  ...
  ```
* Now, weâll create the `InferInput` that weâll be sending to Triton from our data.
  ...
  ```
* Finally, weâre ready to send an inference request to the Triton Inference Server and retrieve the response

  ```
  detection_response = client.infer(model_name="text_detection", inputs=[detection_input])
  ```
* After that, weâll repeat the process with the text recognition model, performing our next processing step, creating the input object, querying the server and finally performing postprocessing and printing the result.
...
You might have noticed that itâs a bit redundant to retrieve the results of the first model only to do some processing and send them right back to Triton. In [Part 5](https://github.com/triton-inference-server/tutorials/blob/main/Conceptual_Guide/Part_5-Model_Ensembles) of this tutorial we explore how you can move more processing steps to the server and execute multiple models in a single network call.

## Model Versioning[#](#model-versioning "Link to this heading")
...
By default Triton serves the âlatestâ model, but the policy to serve different versions of the model is customizable. For more information, [refer this guide](../../../user_guide/model_configuration.md#version-policy).

## Loading & Unloading Models[#](#loading-unloading-models "Link to this heading")
...
The policies can also be set via command line arguments whilst launching the server. For more information, refer [this section](../../../user_guide/model_management.md#model-management) of the documentation.

# Whatâs next?[#](#what-s-next "Link to this heading")

In this tutorial, we covered the very basics of setting up and querying a Triton Inference Server. This is Part 1 of a 6 part tutorial series that covers the challenges faced in deploying Deep Learning models to production. [Part 2](https://github.com/triton-inference-server/tutorials/blob/main/Conceptual_Guide/Part_2-improving_resource_utilization) covers `Concurrent Model Execution and Dynamic Batching`. Depending on your workload and experience you might want to jump to [Part 5](https://github.com/triton-inference-server/tutorials/blob/main/Conceptual_Guide/Part_5-Model_Ensembles) which covers `Building an Ensemble Pipeline with multiple models, pre and post processing steps, and adding business logic`.
