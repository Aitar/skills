...
# PyTorch (LibTorch) Backend[#](#pytorch-libtorch-backend "Link to this heading")
...
is designed to run
[TorchScript](https://pytorch.org/docs/stable/jit.md)
...
## Build the PyTorch Backend[#](#build-the-pytorch-backend "Link to this heading")
...
## Build the PyTorch Backend With Custom PyTorch[#](#build-the-pytorch-backend-with-custom-pytorch "Link to this heading")
...
> [!NOTE]
> Additional details and version information can be found in the containerâs
> [release notes](https://docs.nvidia.com/deeplearning/frameworks/pytorch-release-notes/rel-25-09.md#rel-25-09).
...
## Using the PyTorch Backend[#](#using-the-pytorch-backend "Link to this heading")

### PyTorch 2.0 Models[#](#pytorch-2-0-models "Link to this heading")

PyTorch 2.0 features are available.
However, Tritonâs PyTorch backend requires a serialized representation of the model in the form a `model.pt` file.
The serialized representation of the model can be generated using PyTorchâs
[`torch.save()`](https://docs.pytorch.org/tutorials/beginner/saving_loading_models.md#id1)
...
Triton exposes some flags to control the execution mode of the TorchScript models through the `Parameters` section of the modelâs `config.pbtxt` file.
...
* `INFERENCE_MODE`:
  ...
  [InferenceMode](https://pytorch.org/cppdocs/notes/inference_mode.md) is a new RAII guard analogous to `NoGradMode` to be used when you are certain your operations will have no interactions with autograd.
...
* `INTER_OP_THREAD_COUNT`:
  ...
  > [!TIP]
  > Refer to
  > [CPU Threading TorchScript](https://pytorch.org/docs/stable/notes/cpu_threading_torchscript_inference.md)
...
### Model Instance Group Kind[#](#model-instance-group-kind "Link to this heading")

The PyTorch backend supports the following kinds of
[Model Instance Groups](../user_guide/model_configuration.md#instance-groups)
...
[`torch.set_num_threads(int)`](https://pytorch.org/docs/stable/generated/torch.set_num_threads.md#torch.set_num_threads)

* Key: `NUM_THREADS`
* Value: The number of threads used for intra-op parallelism on CPU.

[`torch.set_num_interop_threads(int)`](https://pytorch.org/docs/stable/generated/torch.set_num_interop_threads.md#torch.set_num_interop_threads)

* Key: `NUM_INTEROP_THREADS`
* Value: The number of threads used for interop parallelism (e.g. in JIT interpreter) on CPU.

[`torch.compile()` parameters](https://pytorch.org/docs/stable/generated/torch.compile.md#torch-compile)
...
* The execution of PyTorch model on GPU is asynchronous in nature.
  See
  [CUDA Asynchronous Execution](https://pytorch.org/docs/stable/notes/cuda.md#asynchronous-execution)
...
* In a multi-GPU environment, a potential runtime issue can occur when using
  [Tracing](https://pytorch.org/docs/stable/generated/torch.jit.trace.md)
  to generate a
  [TorchScript](https://pytorch.org/docs/stable/jit.md)
  model.
  ...
  [Scripting](https://pytorch.org/docs/stable/generated/torch.jit.script.md#torch.jit.script)
  ...
  [model configuration](../user_guide/model_configuration.md#instance-groups)
...
>   [`torch.nn.Module`](https://pytorch.org/docs/stable/generated/torch.nn.Module.md#torch.nn.Module).
