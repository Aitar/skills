...
#### Implicit State Management[#](#implicit-state-management "Link to this heading")
...
the model is using implicit state. The *input\_name* field specifies the name of
the input tensor that will contain the input state. The *output\_name* field
describes the name of the output tensor produced by the model that contains
output state. The output state provided by the model in the *ith*
request in the sequence will be used as the input state in the
*i+1th* request. The *dims* field specifies the dimensions of the
...
and [pytorch\_backend](https://github.com/triton-inference-server/pytorch_backend)
support implicit state.

##### State Initialization[#](#state-initialization "Link to this heading")
...
request is starting, the model sets the âOUTPUT\_STATEâ to be equal to the
âINPUTâ tensor. For non-starting requests, it sets the âOUTPUT\_STATEâ tensor
to the sum of âINPUTâ and âINPUT\_STATEâ tensors.
...
###### Initializing State from Zero.[#](#initializing-state-from-zero "Link to this heading")
...
###### Initializing State from File[#](#initializing-state-from-file "Link to this heading")

For initializing state from file, you need to create a directory named
âinitial\_stateâ under the model directory. The file that contains the initial
state under this directory needs to be provided in the *data\_file* field.
...
#### Scheduling Strategies[#](#scheduling-strategies "Link to this heading")
...
The sequence\_batching section indicates that the model should use the
...
sequence batcher so only those controls are listed. The instance\_group
indicates two instances of the model should be instantiated and
max\_batch\_size indicates that each of those instances should perform
...
![Sequence Batching Example](../_images/sequence_example0.png)
...
![Sequence Batcher Example](../_images/sequence_example1.png)
...
![Sequence Batcher Example](../_images/sequence_example2.png)

##### Oldest[#](#oldest "Link to this heading")
...
the CONTROL\_SEQUENCE\_CORRID control so that it knows which sequence
each inference request in the batch belongs to. The
CONTROL\_SEQUENCE\_READY control is typically not needed because all
...
![Sequence Batching Example](../_images/dyna_sequence_example0.png)
...
![Sequence Batcher Example](../_images/dyna_sequence_example1.png)

### Ensemble Models[#](#ensemble-models "Link to this heading")
...
procedure that involves multiple models, such as âdata preprocessing
-> inference -> data postprocessingâ. Using ensemble models for this
...
image\_preprocess\_model should be used, the content of its input âRAW\_IMAGEâ
is provided by âIMAGEâ tensor, and the content of its output
âPREPROCESSED\_OUTPUTâ will be mapped to âpreprocessed\_imageâ tensor for later
...
without modifying the ensembleâs configuration to exploit the dynamic
...
![Ensemble Example](../_images/ensemble_example0.png)

When an inference request for the ensemble model is received, the ensemble
scheduler will:

1. Recognize that the âIMAGEâ tensor in the request is mapped to input
   âRAW\_IMAGEâ in the preprocess model.
...
3. Recognize the completion of the internal request, collect the output
   tensor and map the content to âpreprocessed\_imageâ which is an unique name
   known within the ensemble.
4. Map the newly collected tensor to inputs of the models within the ensemble.
   In this case, the inputs of âclassification\_modelâ and âsegmentation\_modelâ
...
Unlike other models, ensemble models do not support âinstance\_groupâ field in
