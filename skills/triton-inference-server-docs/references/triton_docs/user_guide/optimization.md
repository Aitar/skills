...
The Performance Analyzer is an essential tool for optimizing your modelâs
performance.
...
## Optimization Settings[#](#optimization-settings "Link to this heading")
...
inception\_graphdef](https://github.com/triton-inference-server/server/blob/main/docs/examples/model_repository/inception_graphdef/config.pbtxt),
...
inference. To see this run perf\_analyzer with request concurrency from
1 to 8.
...
typically applies when perf\_analyzer is running on the same system as
...
instance. So for maximum-batch-size 4 we want to run perf\_analyzer
...
### Model Instances[#](#model-instances "Link to this heading")
...
To specify two instances of the inception\_graphdef model: stop Triton,
...
specific, so you should experiment with perf\_analyzer to determine the
settings that best satisfy your throughput and latency requirements.

## Framework-Specific Optimization[#](#framework-specific-optimization "Link to this heading")
...
### ONNX with TensorRT Optimization (ORT-TRT)[#](#onnx-with-tensorrt-optimization-ort-trt "Link to this heading")
...
use perf\_analyzer to determine the performance of the model using a
...
Triton prints the âStaring endpointsâ message. ONNX model loading can
...
### ONNX with OpenVINO Optimization[#](#onnx-with-openvino-optimization "Link to this heading")

ONNX models running on the CPU can also be accelerated by using
[OpenVINO](https://docs.openvinotoolkit.org/latest/index.md). To
enable OpenVINO optimization for an ONNX model, add the following
lines to the end of the modelâs configuration file.
...
## NUMA Optimization[#](#numa-optimization "Link to this heading")
...
[NUMA](https://www.kernel.org/doc/html/latest/mm/numa.md) configuration for
your system and then assign model instances to different host policies
to exploit these NUMA properties.

### Host Policy[#](#host-policy "Link to this heading")
