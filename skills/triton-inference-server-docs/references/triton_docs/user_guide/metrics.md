...
To change the interval at which metrics are polled/updated, see the `--metrics-interval-ms` flag. Metrics that are updated âPer Requestâ are unaffected by this interval setting. This interval only applies to metrics that are designated as âPer Intervalâ in the tables of each section below:
...
## Inference Request Metrics[#](#inference-request-metrics "Link to this heading")

### Counts[#](#counts "Link to this heading")
...
|  | Inference Count | `nv_inference_count` | Number of inferences performed (a batch of ânâ is counted as ânâ inferences, does not include cached requests) | Per model | Per request |
...
#### Failure Count Categories[#](#failure-count-categories "Link to this heading")
...
> **Note**
>
> Ensemble failure metrics will reflect the failure counts of their composing models as well as the parent model, but currently do not capture the same granularity for the âreasonâ label and will default to the âOTHERâ reason.
>
...
> The `reason=OTHER` reflects that fact that the ensemble doesnât currently capture the specific reason why
> ModelAâs request failed at this time.

#### Pending Request Count (Queue Size) Per-Model[#](#pending-request-count-queue-size-per-model "Link to this heading")
...
âpending request countâ and âqueue sizeâ per-model can be used
...
For those interested in more technical details, the term âpending request countâ
is a bit more accurate than âqueue sizeâ because Triton is highly configurable,
...
* Ensemble Scheduler
  ...
  + The Ensemble Scheduler almost immediately enqueues any requests it receives
    ...
    schedulerâs, however from the ensembleâs perspective, these requests have been
    scheduled.
* Frontends (HTTP/GRPC Servers)

  + Any requests sent from a client to a frontend server in-front of Triton
    may spend some time in the corresponding serverâs code mapping
    protocol-specific metadata to Triton metadata. Though this time is
    generally brief, it will not be considered pending from Tritonâs
    perspective until Triton core has received the request from the frontend.

### Latencies[#](#latencies "Link to this heading")
...
#### Counters[#](#counters "Link to this heading")
...
see the following information exposed by the âTime to First Responseâ histogram
metrics:
...
#### Summaries[#](#summaries "Link to this heading")
...
> They collect information from the [/proc filesystem](https://www.kernel.org/doc/html/latest/filesystems/proc.md) such as `/proc/stat` and `/proc/meminfo`.
...
## Pinned Memory Metrics[#](#pinned-memory-metrics "Link to this heading")
...
## Response Cache Metrics[#](#response-cache-metrics "Link to this heading")
...
2. As of 23.03, additional cache metrics may be reported depending on the
   ...
   being used through Tritonâs [Metrics API](#custom-metrics).

### Triton-reported Response Cache Metrics[#](#triton-reported-response-cache-metrics "Link to this heading")
...
On cache hits, âCache Hit Timeâ indicates the time spent looking up the
response, and âCompute Input Timeâ / âCompute Timeâ / âCompute Output Timeâ
are not recorded.

On cache misses, âCache Miss Timeâ indicates the time spent looking up
the request hash and inserting the computed output tensor data into the cache.
Otherwise, âCompute Input Timeâ / âCompute Timeâ / âCompute Output Timeâ will
...
> management that wouldnât be reflected correctly in the end to end request time.
...
The
[identity\_backend](https://github.com/triton-inference-server/identity_backend/blob/main/README.md#custom-metric-example)
...
### TensorRT-LLM Backend Metrics[#](#tensorrt-llm-backend-metrics "Link to this heading")
...
https://github.com/triton-inference-server/tensorrtllm\_backend?tab=readme-ov-file#triton-metrics

### vLLM Backend Metrics[#](#vllm-backend-metrics "Link to this heading")

The vLLM backend uses the custom metrics API to track and expose specific metrics about
LLMs to Triton:
https://github.com/triton-inference-server/vllm\_backend?tab=readme-ov-file#triton-metrics
