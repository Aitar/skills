...
# Triton Response Cache[#](#triton-response-cache "Link to this heading")
...
on both the server-side, and in the modelâs
[model config](model_configuration.md#response-cache). See the following
sections below for more details.

### Enable Caching on Server-side[#](#enable-caching-on-server-side "Link to this heading")
...
### Enable Caching for a Model[#](#enable-caching-for-a-model "Link to this heading")
...
### Cache Implementations[#](#cache-implementations "Link to this heading")
...
that are used to communicate with a cache implementation of the userâs choice.
...
#### Local Cache[#](#local-cache "Link to this heading")
...
#### Redis Cache[#](#redis-cache "Link to this heading")
...
* The Redis server can be hosted remotely as long as it is accessible by Triton,
  ...
  + This also means that Triton doesnât have to compete with the cache for memory/resource usage.
...
case, and Tritonâs `redis` cache will simply act as a client of your Redis
...
#### Custom Cache[#](#custom-cache "Link to this heading")
...
To put this example together, if the custom cache were named âcustomâ
...
## Deprecation Notes[#](#deprecation-notes "Link to this heading")
...
benefit from caching. The term âsignificantâ here is subjective to the use
...
## Ensemble Model Caching[#](#ensemble-model-caching "Link to this heading")

Top-level requests to ensemble models support caching if all composing models
within the ensemble support caching as well.

Similarly, if a composing model in the ensemble doesnât support caching,
