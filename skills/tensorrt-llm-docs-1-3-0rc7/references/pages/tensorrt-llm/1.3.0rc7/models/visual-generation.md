# Visual Generation (Prototype) — TensorRT LLM

Source: https://nvidia.github.io/TensorRT-LLM/1.3.0rc7/models/visual-generation.html
...
CFG Parallelism (`--cfg_size2`): Splits positive/negative guidance prompts across GPUs.

Ulysses Parallelism (`--ulysses_sizeN`): Splits the sequence dimension across GPUs for longer sequences.
...
Config — User-facing `VisualGenArgs` (CLI / YAML) is merged with checkpoint metadata into `DiffusionModelConfig`.

Pipeline creation & loading — `AutoPipeline` detects the model type from `model_index.json`, instantiates the matching `BasePipeline` subclass, and loads weights (with optional dynamic quantization) and standard components (VAE, text encoder, tokenizer, scheduler).

Execution — `DiffusionExecutor` coordinates multi-GPU inference via worker processes communicating over ZeroMQ IPC.
