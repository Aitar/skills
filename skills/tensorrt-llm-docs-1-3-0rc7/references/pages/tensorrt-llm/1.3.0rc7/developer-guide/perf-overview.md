...
Source: https://nvidia.github.io/TensorRT-LLM/1.3.0rc7/developer-guide/perf-overview.html
...
and shows the throughput scenario under maximum load. The reported metric is `OutputThroughputperGPU(tokens/sec/GPU)`.
...
All performance values are measured in `outputtokenspersecondperGPU`, where `outputtokens` includes the first and all subsequent generated tokens (input tokens are not included).

Data in these tables is taken from the `PerGPUOutputThroughput(tps/gpu)` metric reported by `trtllm-bench`.
The calculations for metrics reported by trtllm-bench can be found in the dataclasses reporting.py and statistics.py.
...
| Sequence Length (ISL/OSL) | B200DEP4 (FP4) | GB200DEP4 (FP4) | H200DEP8 (FP8) |
| --- | --- | --- | --- |
...
unit: `outputtokenspersecondperGPU`

GPT-OSS 120B#

| Sequence Length (ISL/OSL) | B200DEP2 (FP4) | GB200TP1 (FP4) | H200TP1 (FP8) | H100DEP4 (FP8) |
| --- | --- | --- | --- | --- |
...
| Sequence Length (ISL/OSL) | B200TP1 (FP4) | GB200TP1 (FP4) | H200TP1 (FP8) | H100TP1 (FP8) |
| --- | --- | --- | --- | --- |
...
| Sequence Length (ISL/OSL) | B200TP1 (FP4) | GB200TP1 (FP4) | H200TP2 (FP8) | H100TP2 (FP8) |
| --- | --- | --- | --- | --- |
...
| Sequence Length (ISL/OSL) | 1 GPUTP1,PP1 (FP4) | 2 GPUsTP1,PP2 (FP4) |
| --- | --- | --- |
...
| Sequence Length (ISL/OSL) | B200DEP4 (FP4) | GB200DEP4 (FP4) | H200DEP4 (FP8) | H100DEP8 (FP8) |
| --- | --- | --- | --- | --- |
...
| Sequence Length (ISL/OSL) | 4 GPUsDEP2,PP2 (FP4) | 8 GPUsDEP8,PP1 (FP4) |
| --- | --- | --- |
| 1000/1000 | 1,731 | 969 |
| 1024/1024 | 1,732 | 963 |
| 1024/8192 | 644 | 711 |
...
| Sequence Length (ISL/OSL) | B200TP1 (FP4) | GB200TP1 (FP4) |
| --- | --- | --- |
...
| Sequence Length (ISL/OSL) | 2 GPUsDEP2,PP1 (FP4) | 4 GPUsDEP2,PP2 (FP4) | 8 GPUsDEP8,PP1 (FP4) | 1 GPUTP1,PP1 (FP4) |
| --- | --- | --- | --- | --- |
...
| --- | --- | --- |
| Dataset | Create a synthetic dataset | `pythonbenchmarks/cpp/prepare_dataset.py--tokenizer=$model_name--stdouttoken-norm-dist--num-requests=$num_requests--input-mean=$isl--output-mean=$osl--input-stdev=0--output-stdev=0>$dataset_file` |
| Run | Run a benchmark with a dataset | `trtllm-bench--model$model_namethroughput--dataset$dataset_file--backendpytorch--config$llm_options` |
...
In many cases, we also use a higher KV cache percentage by setting `--kv_cache_free_gpu_mem_fraction0.95` in the benchmark command. This allows us to obtain better performance than the default setting of `0.90`. We fall back to `0.90` or lower if out-of-memory errors are encountered.
