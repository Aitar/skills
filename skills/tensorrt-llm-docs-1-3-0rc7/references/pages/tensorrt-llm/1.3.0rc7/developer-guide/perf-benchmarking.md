...
Source: https://nvidia.github.io/TensorRT-LLM/1.3.0rc7/developer-guide/perf-benchmarking.html
...
easier for users to reproduce our officially published performance overview. `trtllm-bench` provides the follows:
...
The following guidance will mostly focus on benchmarks using `trtllm-bench` CLI. To benchmark the OpenAI-compatible `trtllm-serve`, please refer to the run benchmarking with `trtllm-serve` section.
...
To benchmark the OpenAI-compatible `trtllm-serve`, please refer to the run benchmarking with `trtllm-serve` section.
...
For more information about quantization, refer to Quantization and
the support matrix of the supported quantization methods for each network.
...
To benchmark the PyTorch backend (`tensorrt_llm._torch`), use the following command with dataset generated from previous steps. The `throughput` benchmark initializes the backend by tuning against the dataset provided via `--dataset` (or the other build mode settings described above).
...
Use `trtllm-benchprepare-dataset` with LoRA-specific options to generate requests with LoRA metadata:
...
file is present in checkpoints quantized with Model Optimizer
...
`FP16` cache). When running `trtllm-benchthroughput`. The benchmark will select a KV cache quantization that is best suited
...
TensorRT LLM provides the OpenAI-compatible API via `trtllm-serve` command, and `tensorrt_llm.serve.scripts.benchmark_serving` package to benchmark the online server. Alternatively, AIPerf is a comprehensive benchmarking tool that can also measure the performance of the OpenAI-compatible server launched by `trtllm-serve`.
