# KV Cache Architecture — TensorRT LLM

Source: https://nvidia.github.io/TensorRT-LLM/1.3.0rc7/features/auto_deploy/advanced/kv_cache_architecture.html
...
Collected by `InsertCachedAttention` → `cm.add_resource("kv_cache_0",handler)`

Stored in `_resource_lookup` during `initialize_resources()`

Identified as manageable by `_identify_managed_kv_resources()` if compatible with other KV handlers

View assigned via `self._caches["kv_cache_0"]=manager.get_buffers(0,kv_layout="HND")`
