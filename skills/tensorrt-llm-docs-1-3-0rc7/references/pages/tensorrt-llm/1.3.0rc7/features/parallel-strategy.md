# Parallelism in TensorRT LLM — TensorRT LLM

Source: https://nvidia.github.io/TensorRT-LLM/1.3.0rc7/features/parallel-strategy.html
...
DeepSeek-R1: the `fused_A` GEMM is not sharded.

GQA / MQA / MLA: if `num_heads<tensor_parallel_size`, the KV-cache is replicated on every GPU.
...
then set `--configparallel_config.yaml` in `trtllm-serve` or `trtllm-bench`.
