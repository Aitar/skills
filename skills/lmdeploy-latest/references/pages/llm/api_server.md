# OpenAI Compatible Server — lmdeploy

Source: https://lmdeploy.readthedocs.io/en/latest/llm/api_server.html
...
For the sake of convenience, we refer to this service as `api_server`. Regarding parallel services with multiple models, please refer to the guide about Request Distribution Server.
...
The arguments of `api_server` can be viewed through the command `lmdeployserveapi_server-h`, for instance, `--tp` to set tensor parallelism, `--session-len` to specify the max length of the context window, `--cache-max-entry-count` to adjust the GPU mem ratio for k/v cache etc.
...
Connect to a running Kubernetes cluster and deploy the internlm2_5-7b-chat model service with kubectl command-line tool (replace `<yourtoken>` with your huggingface hub token):
...
May use openapi-generator-cli to convert `http://{server_ip}:{server_port}/openapi.json` to java/rust/golang client.
Here is an example:
...
Launch the proxy server through `lmdeployserveproxy`. Get the correct proxy server url.

Launch the script through `torchrun--nproc_per_node2script.pyInternLM/internlm2-chat-1_8b--proxy_urlhttp://{proxy_node_name}:{proxy_node_port}`.Note: Please do not use `0.0.0.0:8000` here, instead, we input the real ip name, `11.25.34.55:8000` for example.
...
When user got `"finish_reason":"length"`, it means the session is too long to be continued. The session length can be
modified by passing `--session_len` to api_server.

When OOM appeared at the server side, please reduce the `cache_max_entry_count` of `backend_config` when launching the service.
