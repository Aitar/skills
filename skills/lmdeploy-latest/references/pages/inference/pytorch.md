# Architecture of lmdeploy.pytorch — lmdeploy

Source: https://lmdeploy.readthedocs.io/en/latest/inference/pytorch.html
...
Get new requests through `RequestManager`. These requests are cached for now.

The `Scheduler` performs scheduling, deciding which cached requests should be processed and allocating resources for them.

`ModelAgent` swaps the caches according to the information provided by the Scheduler, then performs inference with the patched model.

The `Scheduler` updates the status of requests based to the inference results from `ModelAgent`.

`RequestManager` responds to the sender (`EngineInstance`), and the process return to step 1.
