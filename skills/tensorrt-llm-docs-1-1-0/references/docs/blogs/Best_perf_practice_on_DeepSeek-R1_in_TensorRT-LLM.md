Original URL: https://nvidia.github.io/TensorRT-LLM/1.1.0/blogs/Best_perf_practice_on_DeepSeek-R1_in_TensorRT-LLM.md

* How to get best performance on DeepSeek-R1 in TensorRT LLM

# How to get best performance on DeepSeek-R1 in TensorRT LLM[#](#how-to-get-best-performance-on-deepseek-r1-in-tensorrt-llm "Link to this heading")
...
* [How to get best performance on DeepSeek-R1 in TensorRT LLM](#how-to-get-best-performance-on-deepseek-r1-in-tensorrt-llm)
  ...
  + [Prerequisites: Install TensorRT LLM and download models](#prerequisites-install-tensorrt-llm-and-download-models)
    ...
    - [1. Download TensorRT LLM](#download-tensorrt-llm)
    - [2. Download the DeepSeek R1 models](#download-the-deepseek-r1-models)
    - [3. Build and run TensorRT LLM container](#build-and-run-tensorrt-llm-container)
    - [4. Compile and Install TensorRT LLM](#compile-and-install-tensorrt-llm)
    - [5. Optional: Tune GPU clocks](#optional-tune-gpu-clocks)
  ...
  + [Exploring more ISL/OSL combinations](#exploring-more-isl-osl-combinations)
...
## Prerequisites: Install TensorRT LLM and download models[#](#prerequisites-install-tensorrt-llm-and-download-models "Link to this heading")
...
### 1. Download TensorRT LLM[#](#download-tensorrt-llm "Link to this heading")

**You can also find more comprehensive instructions to install TensorRT LLM in this [TensorRT LLM installation guide](https://nvidia.github.io/TensorRT-LLM/installation/build-from-source-linux.md), refer to that guide for common issues if you encounter any here.**
...
### 2. Download the DeepSeek R1 models[#](#download-the-deepseek-r1-models "Link to this heading")
...
### 3. Build and run TensorRT LLM container[#](#build-and-run-tensorrt-llm-container "Link to this heading")
...
### 4. Compile and Install TensorRT LLM[#](#compile-and-install-tensorrt-llm "Link to this heading")
...
You can set the cuda\_architectures to “100-real” if targeting Blackwell only, and “90-real” to target Hopper only to save some build time.
...
### 5. Optional: Tune GPU clocks[#](#optional-tune-gpu-clocks "Link to this heading")
...
### 6. Dataset preparation[#](#dataset-preparation "Link to this heading")

The trtllm-bench tool requires a dataset file to read prompts and output sequence length of each prompt. Format details of this dataset file can be seen in [preparing a dataset](https://nvidia.github.io/TensorRT-LLM/performance/perf-benchmarking.md#preparing-a-dataset).
...
All the benchmarking is done by the trtllm-bench command line tool provided in the TensorRT LLM installation, see [TensorRT LLM Benchmarking](https://nvidia.github.io/TensorRT-LLM/performance/perf-benchmarking.md) for details of this tool.
...
All these commands here are assumed to be running inside the container started by `make -C docker run ...` command mentioned in the [Build and run TensorRT LLM container section](#build-and-run-tensorrt-llm-container)

### B200 min-latency[#](#b200-min-latency "Link to this heading")

Our benchmark results are based on **Batch = 1, ISL = 1K, OSL = 2K, num\_requests = 10 from real dataset**
...
* `trtllm-bench`: A CLI benchmarking utility that aims to make it easier for users to reproduce our officially published. See [TensorRT LLM Benchmarking](https://nvidia.github.io/TensorRT-LLM/performance/perf-benchmarking.md) for details.
...
#### Expected Results[#](#expected-results "Link to this heading")
...
### B200 max-throughput for R1-0528 with FP8 KV cache[#](#b200-max-throughput-for-r1-0528-with-fp8-kv-cache "Link to this heading")
...
#### Expected Result Format[#](#expected-result-format "Link to this heading")
...
### B200 max-throughput for R1 with FP16 KV cache[#](#b200-max-throughput-for-r1-with-fp16-kv-cache "Link to this heading")

Our benchmark results are based on **Batch = 3072, ISL = 1K, OSL = 2K, num\_requests = 49152 from synthetic dataset**.

The results are reproduced with TensorRT LLM commit b6261862419c33d6ce2313aff1e7116067d6037d.

!! Note that the exact command to reproduce numbers can change as the API/options are refactored, the option and numbers here is a reference at given exact commit.

#### Benchmark[#](#id1 "Link to this heading")
...
#### Expected Result Format[#](#id2 "Link to this heading")
...
### H200 min-latency[#](#h200-min-latency "Link to this heading")
...
#### Expected Result Format[#](#id3 "Link to this heading")
...
### H200 max-throughput[#](#h200-max-throughput "Link to this heading")

Our benchmark results are based on **Batch = 1024, ISL = 1K, OSL = 2K, num\_requests = 5120 from real dataset**
To do the benchmark, run the following command:
...
#### Expected Result Format[#](#id4 "Link to this heading")
...
## Exploring more ISL/OSL combinations[#](#exploring-more-isl-osl-combinations "Link to this heading")
...
### WIP: Enable more features by default[#](#wip-enable-more-features-by-default "Link to this heading")
...
### MLA chunked context[#](#mla-chunked-context "Link to this heading")
...
### Out of memory issues[#](#out-of-memory-issues "Link to this heading")
...
Running a High Performance GPT-OSS-120B Inference Server with TensorRT LLM](tech_blog/blog9_Deploying_GPT_OSS_on_TRTLLM.md "previous page")
[next

H200 achieves nearly 12,000 tokens/sec on Llama2-13B with TensorRT LLM](H200launch.md "next page")
...
* [Reproducing steps](#reproducing-steps)
  ...
  + [B200 max-throughput for R1 with FP16 KV cache](#b200-max-throughput-for-r1-with-fp16-kv-cache)
    ...
    - [Expected Result Format](#id2)
  + [H200 min-latency](#h200-min-latency)
    - [Expected Result Format](#id3)
  + [H200 max-throughput](#h200-max-throughput)
    - [Expected Result Format](#id4)
* [Exploring more ISL/OSL combinations](#exploring-more-isl-osl-combinations)
  + [WIP: Enable more features by default](#wip-enable-more-features-by-default)
