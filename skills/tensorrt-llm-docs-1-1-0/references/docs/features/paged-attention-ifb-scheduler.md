Original URL: https://nvidia.github.io/TensorRT-LLM/1.1.0/features/paged-attention-ifb-scheduler.md

* Paged Attention, IFB, and Request Scheduling

# Paged Attention, IFB, and Request Scheduling[#](#paged-attention-ifb-and-request-scheduling "Link to this heading")
...
### `max_batch_size`, `max_seq_len` and `max_num_tokens`[#](#max-batch-size-max-seq-len-and-max-num-tokens "Link to this heading")

![Explain `max_batch_size`, `max_seq_len` and `max_num_tokens`](https://github.com/NVIDIA/TensorRT-LLM/blob/main/docs/source/media/max_bs_toks_len.svg?raw=true)
...
## Chunked Context (a.k.a Chunked Prefill)[#](#chunked-context-a-k-a-chunked-prefill "Link to this heading")
...
[`tensorrt_llm.runtime.KVCacheManager`](https://github.com/NVIDIA/TensorRT-LLM/tree/48b7b5d/tensorrt_llm/runtime/kv_cache_manager.py).
A more efficient C++ implementation is included in the
[Batch Manager](https://github.com/NVIDIA/TensorRT-LLM/tree/48b7b5d/cpp/include/tensorrt_llm/batch_manager).

## The schedulers[#](#the-schedulers "Link to this heading")
...
![TRT-LLM Scheduler Visualization 1](../_images/TRTLLM_Scheduler_Vis_1.svg)
...
![TRT-LLM Scheduler Visualization 2](../_images/TRTLLM_Scheduler_Vis_2.svg)
...
![TRT-LLM Scheduler Visualization 3](../_images/TRTLLM_Scheduler_Vis_3.svg)
...
![TRT-LLM Scheduler Visualization 4](../_images/TRTLLM_Scheduler_Vis_4.svg)
...
## Revisiting Paged Context Attention and Context Chunking[#](#revisiting-paged-context-attention-and-context-chunking "Link to this heading")
...
![TRT-LLM Scheduler Visualization Chunked Context 1](../_images/TRTLLM_Scheduler_Vis_Chunked_Context_1.svg)
...
Overlap Scheduler](overlap-scheduler.md "previous page")
[next

Parallelism in TensorRT LLM](parallel-strategy.md "next page")

On this page

* [In-flight Batching](#in-flight-batching)
  + [`max_batch_size`, `max_seq_len` and `max_num_tokens`](#max-batch-size-max-seq-len-and-max-num-tokens)
    - [`max_batch_size`](#max-batch-size)
    - [`max_seq_len`](#max-seq-len)
    - [`max_num_tokens`](#max-num-tokens)
* [Chunked Context (a.k.a Chunked Prefill)](#chunked-context-a-k-a-chunked-prefill)
* [KV Cache](#kv-cache)
  + [Contiguous KV Cache](#contiguous-kv-cache)
  + [Paged KV Cache](#paged-kv-cache)
* [The schedulers](#the-schedulers)
* [Revisiting Paged Context Attention and Context Chunking](#revisiting-paged-context-attention-and-context-chunking)