...
Source: https://nvidia.github.io/TensorRT-LLM/1.3.0rc7/quick-start-guide.html
...
```text
docker run --rm -it --ipc host --gpus all --ulimit memlock=-1 --ulimit stack=67108864 -p 8000:8000 nvcr.io/nvidia/tensorrt-llm/release:1.3.0rc7
```
...
Expose a port (e.g., 8000) to allow external access to the server from outside the container.
...
Pre-configured settings for deploying popular models with `trtllm-serve` can be found in our deployment guides.
...
You can also directly load pre-quantized models quantized checkpoints on Hugging Face in the LLM constructor.
To learn more about the LLM API, check out the LLM API Introduction and LLM Examples.
...
To learn more about VisualGen, check out the Visual Generation documentation and `examples/visual_gen/`.
...
Model-Specific Deployment Guides - Instructions for serving specific models with TensorRT LLM
